    ...
    $.fn.select_unskin = function (w) {
        return $(this).each(function(i) {
            ...
            if (!s.attr('multiple') && s.parent().hasClass('cmf-skinned-select')) {
                ...
                s.css( { 'opacity': 100, 'z-index': 0,'border': '1px solid #d9d9d9' ,'border-bottom': '1px solid #ccc','border-radius': '2px',
                          'margin-right': '-2px','color': '#666', 'font-family' : 'sans-serif','font-size ' : '12px','top':'2px'} ).unwrap();
             }
        });
    }
}(jQuery));