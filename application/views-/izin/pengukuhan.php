...
<script language="javascript">
	...
	function simpan() {
		...
		var postStr =
			...
			"&pajak=" + pajak;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/izin/simpan_ukuh', postStr, responePOST);
	}
...
<h2 style="margin:30px 5px 25px 30px;">Surat Pengukuhan</h2>
  ...
  <div id="C1" style="background-color: #B3D9F0;"><br />
    <form name="frmPengukuhan" id="frmPengukuhan">
    	<table border="0" id="tab_pengukuhan">
    		<tr>
                <td width="174" style="padding-left:15px;">No. Pengukuhan</td>
            ...
            <tr>
				...
				<td><input type="text" name="usaha" id="usaha" disabled="disabled" size="46px" style="background-color:#FFFFCC;"/></td>
            </tr>
            </table>
            <table>
            <tr>
...
<script language="javascript">
	function cetakData(){
		window.open('<?php echo site_url(); ?>/izin/cetak_ukuh?npwpd='+document.frmPengukuhan.npwpd.value, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	grid.setHeader("ID, No. Pengukuhan, NPWPD Perusahaan, Nama Perusahaan, Alamat Perusahaan, Kewajiban, Jenis_usaha");
	grid.setInitWidths("50,100,120,150,300,80,120");
	...
	grid.loadXML("<?php echo site_url(); ?>/izin/data_pengukuhan");
	grid.init();
				...
				if(confrm) {					
						var postStr =
							...
							"&npwpd=" + document.frmPengukuhan.npwpd.value;		
								//alert(postStr);
							statusLoading();
							dhtmlxAjax.post(base_url+'index.php/izin/delete_ukuhh', postStr, responeDel);	
				}
			} else {
