...
<div align="center">
	<h2>Laporan Realisasi Penerimaan Rekapitulasi Pendapatan SKPD</h2>
...
<div id="gridbox" width="1237px" height="390px" style="margin-bottom:10px;"></div>
</div>
...
function tpdf(){
	...
	window.open('<?php echo site_url();?>/realisasi_pendapatan/tanda_tangan_skpd?full='+periode, '', 'height=700,width=1000,scrollbars=yes');
}
...
function tampil(){
	...
	statusLoading();
	mygrid.clearAll();
	mygrid.loadXML("<?php echo site_url(); ?>/realisasi_pendapatan/data_periode_lap_skpd/"+periode,function() {	       		
		document.bln.pdf.disabled = false;
        statusEnding();
	});
...
mygrid.setHeader("No., Uraian, Target <br>Tahun Anggaran,Penerimaan<br>Bulan Ini,Penerimaan<br>Bulan Lalu,Penerimaan<br>Sampai Bulan Ini,Presentase,Sisa <br>Target",//27
...
mygrid.attachHeader("1, 2, 3, 4, 5, 6, 7, 8",
...
mygrid.setInitWidths("40,300,140,170,170,170,80,160");
mygrid.setColAlign("center,left,right,right,right,right,center,right");
mygrid.setColTypes("ron,ro,ron,ron,ron,ron,ron,ron");
...
mygrid.attachFooter("<div style=text-align:center>J U M L A H</div>,#cspan,<div style=text-align:right id=s0>{#stat_total}</div>,<div style=text-align:right id=s0>{#stat_total}</div>,<div style=text-align:right id=s1>{#stat_total}</div>,<div style=text-align:right id=s2>{#stat_total}</div>,<div style=text-align:right id=s3>{#stat_total}</div>,<div style=text-align:right id=s4>{#stat_total}</div>,<div style=text-align:right id=s5>{#stat_total}</div>");
mygrid.init();
</script>