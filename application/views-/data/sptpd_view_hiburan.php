...
<script language="javascript">
	...
	function simpan2(){	
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptpd_hiburan/simpan', postStr, function(loader) {
			result = loader.xmlDoc.responseText;
			arr = result.split("/");
			...
			if(name=='dispenda'){
				...
				if(cnf) {
					var postStr =
						"sptpd_1=" + document.frmData.sptpd_1.value +
						"&sptpd_2=" + document.frmData.sptpd_2.value;
					dhtmlxAjax.post(base_url+'index.php/sptpd_hiburan/hapus', postStr, function(loader) {
						result = loader.xmlDoc.responseText;
						alert(result);
						gridData.clearAll();
						loadData();
    ...
    <br />
   	  <table width="888">
            <tr>
              ...
              <td colspan="2"><input name="sptpd_1" type="text" id="sptpd_1" size="10" readonly style="background-color:#FFFFCC;"> 
                / 
                <input name="sptpd_2" type="text" id="sptpd_2" size="5" readonly style="background-color:#FFFFCC;"></td>
            </tr>
            <tr>
            ...
            <tr>
                <td width="228" style="padding-left:15px;">NPWPD Usaha</td>
                <td><input type="hidden" name="id" id="id" size="35" /><input type="hidden" name="sptpd" id="sptpd" size="35" />
                <input type="text" name="npwpd" id="npwpd" size="35" style="text-transform:uppercase; background-color: #FFCC99;" disabled  />
            ...
            <tr>
              ...
              <td><input type="text" name="alamat" id="alamat" size="35" readonly style="background-color:#FFFFCC; text-transform:uppercase;" disabled="disabled"></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <option value="<?php echo $userPetugas; ?>"><?php echo $namaPetugas; ?></option>
           	...
           	<tr>
              	<td style="padding-left:15px;">Golongan Hiburan</td>
                <td colspan="2"><select name="gol" id="gol" onChange="cgol()" disabled="disabled">
                <option value=""></option>
                <?php 
				foreach($gresto->result() as $rs) {
						echo "<option value=".$rs->id."-".$rs->kd_rek.">".$rs->nm_rek."</option>";
					}
				?>
                </select></td>
            </tr>
...
<div id="objBrg" style="display:none;">
  ...
  <tr>
    ...
    <td width="216"><select name="parameter" id="parameter">
      <option value="no_npwpd">NPWPD Usaha</option>
      <option value="nama_perusahaan">Nama Usaha</option>
    </select>    </td>
...
<script language="javascript">	
	function cgol(){
		if(document.frmData.gol.value=="") {
			alert("Golongan Hiburan Tidak Boleh Kosong");
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptpd_hiburan/tarif_hib', postGol, respJns);			
	}
	...
	function cetak1() {
		...
		var c = a+'/'+b;
		window.open('<?php echo site_url(); ?>/sptpd_hiburan/cetak?sptpd='+c, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	function setGolTarif(){
		//Get list golongan
		var golArray = new Array();
		var gol = document.getElementById('gol');
		for(i = 0; i < gol.options.length; i++){
			golArray[i] = gol.options[i].value;
		}
		
		//getKode jenis
		var npwpd = document.frmData.npwpd.value;
		var kodeJenis = npwpd.substr(8,2);
		
		//get idGolongan
		var indeksGol = 0;
		for(i = 0; i < gol.options.length; i++){
			golArray[i] = gol.options[i].value;
			if(golArray[i].substr(12,2)==kodeJenis){
				indeksGol = i;
			}
		}
		...
		//get nama usaha
		var namaUsaha = document.frmData.nama.value.toUpperCase();
		
		//cek refleksi atau panti pijat
		var posisiKata = namaUsaha.indexOf("REFLE");
		if(posisiKata>=0) document.getElementById('gol').selectedIndex = 7;
		else document.getElementById('gol').selectedIndex = indeksGol;
	...
	function cariDataWP() {
		...
		statusLoading();
		gridWP.clearAll();
		gridWP.loadXML("<?php echo site_url(); ?>/sptpd_hiburan/cariDataWP/"+kata_kunci+"/"+document.frmSrc.parameter.value,function() {  statusEnding(); });
		//gridWP.loadXML(base_url+"index.php/sptpd_hiburan/cariDataWP/"+kata_kunci+"/"+document.frmSrc.parameter.value,function() {  statusEnding(); });
	}
	
	// Wajib Pajak
	gridData = new dhtmlXGridObject('gridData');
	gridData.setImagePath("<?php echo base_url(); ?>/assets/codebase_grid/imgs/");
	gridData.setHeader("No.SPTPD,Masa Pajak,Masa Pajak,Tahun,NPWPD,Nama Uasaha,Alamat,Tgl Terima,Petugas,Setoran,cara,masa1,masa2,jenis,insidentil,gol,Dasar Pengenaan,tarif,ket pajak");//19
	gridData.attachHeader("#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter");
	...
	function loadData() {
		gridData.clearAll();
		gridData.loadXML("<?php echo site_url(); ?>/sptpd_hiburan/mainData");
		//gridData.loadXML(base_url+"index.php/sptpd_hiburan/mainData",function() { statusEnding(); });
	}
	statusEnding();
</script>
</div>