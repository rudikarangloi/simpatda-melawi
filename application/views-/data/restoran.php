...
<script language="javascript">
	...
	function popupform(myform, windowname){
		...
		window.open('', windowname, 'height=2000,width=1000,scrollbars=yes');
	...
	function simpan2(){	
		...
		var postStr =
			...
			"&meja=" + document.frmData.meja.value+
			"&kursi=" + document.frmData.kursi.value+
			"&jml_pengunjung=" + document.frmData.jml_pengunjung.value+
			"&kas_register=" + document.frmData.kas_register.value+
			"&pembukuan=" + document.frmData.pembukuan.value;
	...
	function disableData() {
		...
		document.frmData.meja.disabled = true;
		document.frmData.kursi.disabled = true;
		document.frmData.jml_pengunjung.disabled = true;
		document.frmData.kas_register.disabled = true;
		document.frmData.pembukuan.disabled = true;
	...
	function enableData() {
		...
		document.frmData.meja.disabled = false;
		document.frmData.kursi.disabled = false;
		document.frmData.jml_pengunjung.disabled = false;
		document.frmData.kas_register.disabled = false;
		document.frmData.pembukuan.disabled = false;
...
<style>
    ...
    <div id="a_tabbar" style="width:1000px; height:700px; margin-left:30px;"></div>
    ...
    <br />
   	  <table width="888">
            ...
            <tr>
           	  ...
           	  <td colspan="2"><input name="tgl_terima" type="text" disabled="disabled" id="tgl_terima" value="<?php echo $tanggal; ?>" size="8"></td>
   	    </tr>
           	<tr>
			...
			<tr>
           	  <td style="padding-left:15px;">Meja yang tersedia</td>
              <td><input style="text-align:right" type="text" name="meja" id="meja" size="8" disabled/></td>
   	    	</tr>
			<tr>
           	  <td style="padding-left:15px;">Jumlah kursi</td>
              <td><input style="text-align:right" type="text" name="kursi" id="kursi" size="8" disabled/></td>
   	    	</tr>
			<tr>
           	  <td style="padding-left:15px;">Jml. Pengunjung/hari</td>
              <td><input style="text-align:right" type="text" name="jml_pengunjung" id="jml_pengunjung" size="10" disabled/></td>
   	    	</tr>
			<tr>
						<td style="padding-left:15px;" valign="top">Menggunakan kas register</td>
						<td><select id="kas_register" name="kas_register" disabled/>
							<option></option>
			...
			<tr>
						<td style="padding-left:15px;" valign="top">Mengadakan pembukuan / pencatatan</td>
						<td><select id="pembukuan" name="pembukuan" disabled/>
							<option></option>
							<option value="Ya">Ya</option>							
							<option value="Tidak">Tidak</option>	
            ...
            <tr>
              	<td style="padding-left:15px;">Masa Pajak</td>
                <td colspan="2"><input type="text" name="txttglmasapajak1" id="txttglmasapajak1" size="12" disabled/>&nbsp;
			 <i>s.d</i> <input type="text" name="txttglmasapajak2" id="txttglmasapajak2" size="12" disabled/></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                &nbsp;<input type="radio" name="sen" id="sen2" />Persentase Pajak</td>
            </tr>
            <tr>
...
<div id="objBrg" style="display:none;">
  ...
  <tr>
		...
		<td width="417"><input type="button" name="button2" id="button2" value="CARI" style="height:30px; width:80px;" onClick="cariDataWP();">
		<input type="button" name="button3" id="button3" value="TUTUP" style="height:30px; width:80px;" onClick="closeBrg();"></td>
   </tr>
  <tr>
...
<script language="javascript">	
	...
	function cetak1() {
		...
		var c = a+'/'+b;
		window.open('<?php echo site_url(); ?>/restoran/cetak2?sptpd='+c, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	gridWP.setHeader("id,NPWPD,Nama Badan Usaha,Alamat,Pemilik");
	gridWP.setInitWidths("120,120,200,200,180");
	...
	gridWP.attachEvent("onRowDblClicked", function(id) {
		document.frmData.npwpd.value = gridWP.cells(id,1).getValue();
		document.frmData.nama.value = gridWP.cells(id,2).getValue();
		document.frmData.alamat.value = gridWP.cells(id,3).getValue();
		
		//Tentukan golongan dan tarif pajak
		arrayNPWPD = document.frmData.npwpd.value.split(".");
		document.frmData.gol.value = "4.1.1.02."+arrayNPWPD[2];
		cgol();
		
		closeBrg();
	});
	gridWP.setColumnHidden(0,true);
	...
	/* function cariDataWP() {
		...
		kata_kunci = kata_kunci.replace(/ /g, "%20");
		statusLoading();
		gridWP.clearAll();	
		gridWP.loadXML("<?php echo site_url(); ?>/restoran/cariDataWP/"+kata_kunci+"/"+document.frmSrc.parameter.value,function() {  statusEnding(); });
		//gridWP.loadXML(base_url+"index.php/restoran/cariDataWP/"+kata_kunci+"/"+document.frmSrc.parameter.value,function() {  statusEnding(); });
	} */
	function cariDataWP() {
		op = document.frmSrc.parameter.value;
		if(op==1||op==2){
			if(document.frmSrc.keyword.value=="") {
			alert("Value Pencarian Tidak Boleh Kosong");
			document.frmSrc.keyword.focus();
			return;
			}
		}
		//alert("Password anda salah");
		var  nilai1 = document.frmSrc.keyword.value;
		nilai = nilai1.replace(/%20/gi, " ");
		gridWP.clearAll();
		gridWP.loadXML("<?php echo site_url(); ?>/restoran/cariDataWP/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	gridData.setHeader("No.SPTPD,Masa Pajak,Masa Pajak,Tahun,NPWPD,Nama Usaha,Alamat,Tgl Terima,Petugas,Dasar Pengenaan,cara,masa1,masa2,jenis,insidentil,gol,meja,kursi,jml_pengunjung,register,pembukuan,setoran,tarif,ket pajak");//23
	gridData.attachHeader("#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter");
	gridData.setInitWidths("120,80,80,80,100,150,200,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100");
	gridData.setColAlign("left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left");
	gridData.setColSorting("str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str");
	gridData.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ron,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ron,ron,ro");
	//gridData.setColumnHidden(0,true);
	gridData.setNumberFormat("0,000",9,",","."); 
	gridData.setNumberFormat("0,000",21,",","."); 
	gridData.attachEvent("onRowSelect", function(id) {
		...
		document.frmData.meja.value = gridData.cells(id,16).getValue();
		document.frmData.kursi.value = gridData.cells(id,17).getValue();
		document.frmData.jml_pengunjung.value = gridData.cells(id,18).getValue();
		document.frmData.kas_register.value = gridData.cells(id,19).getValue();
		document.frmData.pembukuan.value = gridData.cells(id,20).getValue();
		...
		document.frmData.jumlah.value = format_number(gridData.cells(id,21).getValue());
		...
		document.frmData.tarif.value = format_number(gridData.cells(id,22).getValue());
		...
		if(gridData.cells(id,18).getValue()=='BELUM TERMASUK PAJAK') {
			document.frmData.sen2.checked = true;
		}
	...
	function loadData() {
		gridData.clearAll();
		gridData.loadXML("<?php echo site_url(); ?>/restoran/mainData");
		//gridData.loadXML(base_url+"index.php/restoran/mainData",function() { statusEnding(); });
	}
	statusEnding();
</script>
</div>