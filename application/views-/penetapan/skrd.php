  ...
  <script language="javascript">
	...
	function simpan() {
		...
		var postStr =
			...
			"&perhitungan=" + document.frmSKPD.perhitungan.value +
			"&nama=" + document.frmSKPD.nama.value +
			"&karcis1=" + document.frmSKPD.karcis1.value +
			"&ttdid=" + document.frmSKPD.ttd_skpd.value +
			"&karcis2=" + document.frmSKPD.karcis2.value +
			"&jml_karcis=" + document.frmSKPD.jml_karcis.value +
	...
	function hapus() {
		...
		if (name!=null){
			if(name=='dispenda'){
				...
				if(cnf) {
					var postStr =
						"nota=" + document.frmSKPD.nota_hitung.value +
						"&skpd_1=" + document.frmSKPD.skpd_1.value +
						"&skpd_2=" + document.frmSKPD.skpd_2.value;
					statusLoading();
...
body{
  ...
  <h2 style="margin:30px 5px 25px 30px;">SKRD - Surat Ketetapan Retribusi Daerah</h2>
  <div id="a_tabbar" style="width:1000px; height:610px; margin-left:30px;"></div>
  <div id="C1" style="background-color: #B3D9F0">
    <form name="frmSKPD" id="frmSKPD" action="<?php echo site_url(); ?>/skrd/cetak" method="post" onsubmit="popupform(this, 'skpd')" ><input name="skpd" type="hidden" id="skpd" size="8" /><br />
     <table width="888" cellspacing="0">
        ...
        <tr>
         ...
         <td style="padding-left:19px;" align="right">No Seri Karcis</td>
        ...
        <tr>
          ...
          <td colspan="1" align="right">Jumlah Karcis</td>
		  <td><input type="text" name="jml_karcis" id="jml_karcis" readonly="readonly" disabled="disabled" size="15" />
        </tr>
        <tr>
		  <td style="padding-left:15px;">Kriteria Perhitungan</td>
          <td><input type="text" name="perhitungan" id="perhitungan" readonly="readonly" disabled="disabled" size="25" />
          
        </tr>
		<tr>
           <td style="padding-left:15px;">Tanggal SKRD</td>
			<td width="176"><input name="tgl" type="text" id="tgl" size="15" disabled="disabled" /></td>
        </tr>
		<tr>
			<td style="padding-left:15px;">Tahun</td>
			<td colspan="2"><select name="tahun" id="tahun" onchange="cmp()" disabled="disabled">
              <?php echo $tahun; ?>
            </select>		
			</td>
        </tr>
        ...
        <tr>
          <td style="padding-left:15px;"></td>
			<td rowspan="2"><textarea name="ket_skpd" id="ket_skpd" cols="25" rows="4"></textarea></td>
        </tr>
...
<br /><table width="790" border="0">
	<tr>
		<td style="padding-left:17px;">Cari data berdasarkan 
      	<select name="parameter" id="parameter" >
        	<option value="npwpd">No.SPTRD</option>
            <option value="no_nota">SKPD Pengelola</option>
        </select></td>
  ...
  <script language="javascript">
	...
	/* function ubahKel(){
  	...
  	function cet(){
		...
		window.open('<?php echo site_url(); ?>/skrd/cetak?skpd='+gabung, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	gridWP.setHeader("No. SPTRD,Tahun,Kriteria Hitung,Jumlah Karcis,SKPD Pengelola,Tanggal,No Karcis 1,No Karcis 2,Jumlah"); //10
	gridWP.setInitWidths("120,120,120,200,200,180,80,80,80");
	gridWP.setColAlign("left,left,left,left,left,left,left,left,left");
	...
	gridWP.attachEvent("onRowDblClicked", function(id) {
		...
		document.frmSKPD.jml_karcis.value = gridWP.cells(id,3).getValue();
		document.frmSKPD.perhitungan.value = gridWP.cells(id,2).getValue();
		document.frmSKPD.nama.value = gridWP.cells(id,4).getValue();
		document.frmSKPD.karcis1.value = gridWP.cells(id,6).getValue();
		document.frmSKPD.karcis2.value = gridWP.cells(id,7).getValue();
		...
		document.frmSKPD.tahun.value = gridWP.cells(id,1).getValue();
		document.frmSKPD.txtjml.value = format_number(gridWP.cells(id,8).getValue());
		...
		arrTgl_1 = gridWP.cells(id,5).getValue().split("-");
	...
	gridData.setHeader("Kode Rekening,Nama Rekening,Jumlah Karcis,Tarif,Kenaikan,Denda,Bunga,Kompensasi,Jumlah,SKPD");
	gridData.setInitWidths("120,120,120,80,80,80,80,80,80,80");
	...
	grid.setHeader("No SKRD,SKPD Pengelola,Kriteria Hitung,Tahun,Tanggal Terima ,No SPTRD,Jumlah,Jumlah Karcis,No Karcis 1,No Karcis 2,Id TTD,KET"); //11
	...
	grid.setInitWidths("100,150,100,100,100,100,100,100,100,100,100,100");
	grid.setColAlign("left,left,center,left,left,left,center,center,center,center,left,left");
	grid.setColTypes("ro,ro,ro,ro,ro,ro,ron,ron,ro,ro,ro,ro");
	...
	grid.attachEvent("onRowDblClicked", function(id) {
		...
		document.frmSKPD.skpd_1.value = arr[0];
		document.frmSKPD.skpd_2.value = arr[1]+'/'+arr[2];
		document.frmSKPD.nama.value = grid.cells(id,1).getValue();
		document.frmSKPD.perhitungan.value = grid.cells(id,2).getValue();
		document.frmSKPD.tahun.value = grid.cells(id,3).getValue();
		document.frmSKPD.tgl.value = grid.cells(id,4).getValue();
		document.frmSKPD.nota_hitung.value = grid.cells(id,5).getValue();
		document.frmSKPD.txtjml.value = format_number(grid.cells(id,6).getValue());
		document.frmSKPD.jml_karcis.value = grid.cells(id,7).getValue();
		document.frmSKPD.karcis1.value = grid.cells(id,8).getValue();
		document.frmSKPD.karcis2.value = grid.cells(id,9).getValue();
		document.frmSKPD.ttd_skpd.value = grid.cells(id,10).getValue();
		document.frmSKPD.ket_skpd.value = grid.cells(id,11).getValue();
		loadItemPajak();
		enabledButton();
	});
	grid.init();
