...
<script language="javascript">
	...
	function simpan() {
		...
		var postStr =
			...
			"&nomor=" + document.frmPengukuhan.nomor.value +
			"&tunggakan=" + tunggakan +
			"&pajak=" + pajak;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/tegur_skpd2/simpan', postStr, responePOST);
	}
	...
	function kosongfrmData() {
		...
		document.frmPengukuhan.nomor.value = "";
	...
	function barus(){
		...
		document.frmPengukuhan.nomor.disabled = false;
...
<h2 style="margin:30px 5px 25px 30px;">Surat Teguran I</h2>
  ...
  <div id="C1" style="background-color: #B3D9F0;"><br />
    <form name="frmPengukuhan" id="frmPengukuhan">
    	<table border="0" id="tab_pengukuhan">
    		<tr>
                <td width="174" style="padding-left:15px;">Id. Teguran</td>
                <td width="400"><input type="hidden" name="id" id="id" size="35px" style="text-transform:uppercase; background-color: #FFFFCC;" disabled="disabled"><input type="text" name="ukuh" id="ukuh" size="35px" style="text-transform:uppercase; background-color: #FFFFFF;"></td>
            </tr>
			<tr>
              	<td style="padding-left:15px;">No. Surat Teguran</td>
				<td><input type="text" name="nomor" id="nomor"  disabled="disabled" size="9px" style="background-color:#white;"/></td>
            </tr>
            <tr>
			 ...
			 <tr>
    			<td style="padding-left:15px;">Tanggal Surat</td>
				<td><input type="text" name="tgl" id="tgl" disabled="disabled" size="15px" style="background-color:#FFFFFF;"/></td>
         	</tr>
            <tr>
			...
			<tr>
				...
				<td><textarea name="tunggakan" cols="45" rows="5" disabled="disabled" id="tunggakan"></textarea></td>
            </tr>
            <tr>
...
<!--<br />
    ...
    <div style="padding:0 0px 0px 0px;">
   		<div id="gridContent" height="500px" width="995px" style="margin-bottom:50px;"></div>
        <div id="pagingArea" width="450px" style="background-color:white;"></div>
	</div> 
</div>
...
<script language="javascript">
	function cetakData(){
		...
		var tmbs_satpp = document.getElementById('tmbs_satpp').checked;
            window.open('<?php echo site_url(); ?>/tegur_skpd2/cetak?id_surat_tegur='+document.frmPengukuhan.id.value+'&tmbs_wali='+tmbs_wali+'&tmbs_insp='+tmbs_insp+'&tmbs_satpp='+tmbs_satpp, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	grid.setHeader("ID, Id. Teguran,No. Teguran, NPWPD Perusahaan, Tanggal Surat, Nama Perusahaan, Alamat Perusahaan, Kewajiban, Pemilik/Pimpinan, Tunggakan, Jenis_usaha");
	grid.setInitWidths("50,110,110,120,120,150,300,80,100,150,120");
	...
	grid.setColAlign("center,center,center,center,center,left,left,left,center,center,left");
	...
	grid.init();
	//grid.loadXML("<?php echo site_url(); ?>/tegur_skpd2/data");
	refreshData()
	...
	function selectedOpenData(id) {
		...
		//ganti teks tunggakan "&amp;" jadi "&"
		var tunggakan = grid.cells(id,9).getValue();
		...
		document.frmPengukuhan.nomor.value = grid.cells(id,2).getValue();
		document.frmPengukuhan.npwpd.value = grid.cells(id,3).getValue();
		document.frmPengukuhan.tgl.value = grid.cells(id,4).getValue();
		document.frmPengukuhan.nama.value = grid.cells(id,5).getValue();
		document.frmPengukuhan.alamat.value = grid.cells(id,6).getValue();
		menu = grid.cells(id,7).getValue();
        document.frmPengukuhan.usaha.value = grid.cells(id,8).getValue();
		document.frmPengukuhan.pemilik.value = grid.cells(id,8).getValue();
				...
				if(confrm) {					
						var postStr =
							...
							"&npwpd=" + document.frmPengukuhan.npwpd.value;		
								//alert(postStr);
							statusLoading();
							dhtmlxAjax.post(base_url+'index.php/tegur_skpd2/delete', postStr, responeDel);	
				}
			} else {
