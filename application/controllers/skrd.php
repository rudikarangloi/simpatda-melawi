...
class skrd extends MY_Controller {
	...
	public function frmskpd($idtype="") {
		...
		$data['tahun'] = $this->msistem->tahun();
		$this->load->view('penetapan/skrd',$data);
	}
	...
	public function cariData($kata_kunci="",$parameter="",$kode_sptpd="") {
		...
		if($kata_kunci != '0'):
			...
			if($this->session->userdata('username')=='admin'){
				...
				$muncul = "and sptrd.author = '".$this->session->userdata('username')."'";
			}
		
		$grid = new GridConnector($this->db->conn_id);
		$grid->dynamic_loading(100); 
		$grid->render_sql("SELECT no_sptrd, tahun, kriteria_htg,jml_karcis,skpd_pengelola,tgl_diterima,no_karcis1,no_karcis2,jml_bayar FROM sptrd
						  where status ='0' ".$muncul." ","id","no_sptrd, tahun, kriteria_htg ,jml_karcis, skpd_pengelola, tgl_diterima, no_karcis1, no_karcis2, jml_bayar");
	}
	...
	public function simpan() {
		...
		$this->db->update('sptrd', $upd, array('no_sptrd' => $_POST['nota_hitung']));
		...
		if($this->input->post('skpd_1')=="") {
			...
			$dataIns = array_merge($data,array('no_skrd' => $no_skpd,'created' => date('Y-m-d H:i:s')));
			$result = $this->db->insert('skrd', $dataIns);
		} else {
			$no_skpd = $this->input->post('skpd_1')."/".$this->input->post('skpd_2');
			$this->db->delete('skrd_child',array('skrd' => $no_skpd));
			$dataUpdate = array_merge($data,array('author' => $this->session->userdata('username'),'modified' => date('Y-m-d H:i:s')));
			$result = $this->db->update('skrd',$dataUpdate,array('no_skrd' => $no_skpd));
		}
		echo $no_skpd;
	}
	
	public function generateNo($thn="",$kode="") {
		$sql = $this->db->query("select MAX(SUBSTRING_INDEX(no_skrd,'/',1)) as max from skrd where LEFT(tgl_terima,4) = '".$thn."' and kode = '".$kode."'");
		$r = $sql->row();
		$jml = $r->max + 1;
		if(strlen($jml)==1) {
			$jml = '00000'.$jml;
	...
	function mainData() {
		...
		$grid = new GridConnector($this->db->conn_id);
		/* $grid->dynamic_loading(100);  */
		$grid->render_sql("SELECT id, no_skrd, skpd_pengelola, kriteria_htg, tahun, tgl_terima, 
		  no_sptrd, jumlah, jml_karcis, no_karcis1, no_karcis2, id_ttd, ket_skpd FROM skrd ORDER BY no_skrd DESC","id","no_skrd,skpd_pengelola ,kriteria_htg, tahun, tgl_terima,
          no_sptrd, jumlah, jml_karcis, no_karcis1, no_karcis2, id_ttd, ket_skpd");
	}
	
	function hapus() {
		$no_skpd = $this->input->post('skpd_1')."/".$this->input->post('skpd_2');
		$s = $this->db->query("select count(id) as jml from ssrd where no_skrd='".$no_skpd."'")->row();
		$we = $s->jml; 
		//fadil
		if($we==0){
			$upd = array (
			...
			$this->db->update('sptrd', $upd, array('no_sptrd' => $this->input->post('nota')));
			$this->db->delete('skrd',array('no_skrd' => $no_skpd));
			$this->db->delete('skrd_child',array('skrd' => $no_skpd));
	...
	function dataItemPajak1($s=""){
		...
		$wo = $this->db->query("select kd_rek,nm_rek,dp,tarif,jumlah from sptrd_child where no_sptrd='".$spt."'");
	...
	public function cetak_minerba() {		
		...
		$report .=
			...
			//$child = $this->db->query("select * from skpd_child where skpd ='".$no."'");
			$child = $this->db->query("SELECT a.*,c.volume,c.harga_pasar,b.omset FROM skpd_child a INNER JOIN sptpd_galianc_d1 c ON a.dp=c.dp INNER JOIN sptpd_galianc b ON c.sptpd=b.no_sptpd  WHERE c.sptpd  ='".$no_sptpd."'");
