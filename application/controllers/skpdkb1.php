...
class skpdkb extends MY_Controller {
	...
	public function lihat() {
		...
		$w = $this->db->query("select setoran from view_setoran where no_nota='".$s."'")->row();
		if($w==NULL){
			$total_setoran = 0;
		} else {
		...
		$q = $this->db->query("select dp,jumlah from nhp_child where no_nota='".$s."'")->row();
		$ketetapan = $q->dp;
		$jumlah = $q->jumlah;
			//now
			$d = date('d/m/Y');
	...
	function delete() {
		...
		$s = $this->db->query("select count(id) as jml from sspd where no_skpd='".$no_skpdkb."'")->row();
		$we = $s->jml; 
		if($we==0){
...
INNER JOIN view_perusahaan ON skpdkb.npwpd = view_perusahaan.npwpd_perusahaan WHERE skpdkb.no_skpdkb='".$_GET['no_skpdkb']."'")->row();
		...
		$child = $this->db->query("select kd_rek, nm_rek,jumlah from skpdkb_child where skpdkb ='".$no."'")->row();
		$dp = number_format($list->dasar_pengenaan,2,",",".");
		$hutang = number_format($child->jumlah,2,",",".");
