...
$terima2 = $query2->jl; */
	...
	function lap_pad2(){
		$data['pajak'] = $this->db->query("select kode_sptpd, nama_sptpd from master_sptpd WHERE kode_sptpd !='RET' AND kode_sptpd !='PLL'");
		$this->load->view('report_pajak/lap_pad2', $data);
	}
	
	function cetak_lap_pad2(){
		$p2 = $_GET['full'];
		$t = explode('|',$p2);
		$tgl_awal = $t[0];
		$tg = explode('-',$tgl_awal);
		...
		$tg2 = $tg[0];
		/*hw*/
		$awal = $this->msistem->v_bln($tg1);
		$prd1 = $tg[2]." ".$awal." ".$tg[0];
		$tgl_akhir = $t[1];
		
		/*$akhir 				= $this->msistem->view_tanggal($tg2);*/
		$marginBawah = $t[2];
		$cetak = $t[2];
		...
		/*if($awal==$akhir){
			$tanggal=strtoupper($awal);
		}else{
			$tanggal= strtoupper($awal).' - '.strtoupper($akhir).'';
		}*/
		...
		$report .=
			'<table border="1" cellpadding="2" cellspacing="0">
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:9px">
					...
					<td width="280" rowspan="2" valign="center" align="center"><strong>Uraian</strong></td>
					<td width="140" rowspan="2" valign="center" align="center"><strong>Target</strong></td>
					<td width="280" colspan="2" valign="center" align="center"><strong>Realisasi Penerimaan</strong></td>
					<td width="150" rowspan="2" valign="center" align="center"><strong>Jumlah S/D Hari Ini</strong></td>
					</tr>
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:9px">
					<td width="140"  valign="center" align="center"><strong>S/D Hari Lalu</strong></td>
					<td width="140" valign="center" align="center"><strong>Hari Ini</strong></td>
				</tr>
				';
				
		$query2 =  $this->db->query("SELECT a.kd_rek,c.nm_rek,a.target_realisasi,b.kode_rekening,b.kemarin,b.hari_ini FROM(
					SELECT LEFT(a.kd_rek,3) AS kd_rek ,SUM(target_realisasi) target_realisasi FROM master_rekening a
					WHERE SUBSTR(kd_rek,1,3) ='4.1' AND a.status_aktif='1')a
					LEFT JOIN (SELECT IFNULL(LEFT(b.kode_rekening,3),0) AS kode_rekening,
					SUM(CASE WHEN tanggal <'$tgl_awal'  THEN setoran ELSE 0 END) AS kemarin ,
					SUM(CASE WHEN tanggal ='$tgl_awal'  THEN setoran ELSE 0 END) AS hari_ini 
					FROM sspd a
					LEFT JOIN sspd_detail b ON a.no_sspd = b.no_sspd
					where year(tanggal)='$tg[0]'
					GROUP BY LEFT(b.kode_rekening,3))b
					ON a.kd_rek=b.kode_rekening 
					LEFT JOIN(
					SELECT kd_rek,nm_rek FROM master_rekening WHERE LENGTH(kd_rek)='3') c
		...
		$query = "SELECT * FROM(
					...
					WHERE LENGTH(kd_rek)='11' AND LEFT(kd_rek,5)='4.1.1' AND status_aktif='1' AND jns_pajak!='12')a
					LEFT JOIN (SELECT b.kode_rekening,
					SUM(CASE WHEN tgl_bayar <'$tgl_awal'  THEN b.jumlah ELSE 0 END) AS kemarin ,
					SUM(CASE WHEN tgl_bayar ='$tgl_awal'  THEN b.jumlah ELSE 0 END) AS hari_ini 
					FROM sspd a
					LEFT JOIN sspd_detail b ON a.no_sspd = b.no_sspd
					INNER JOIN pembayaran d ON a.no_sspd=d.no_sspd
					where year(tgl_bayar)='$tg[0]'
					GROUP BY kode_rekening)b
					ON a.kd_rek=b.kode_rekening
					UNION ALL
					SELECT * FROM(
					...
					WHERE LENGTH(kd_rek)='11' AND LEFT(kd_rek,5)='4.1.1' AND status_aktif='1' AND jns_pajak='12')a
					LEFT JOIN (SELECT b.kd_rek,
					SUM(CASE WHEN tgl_diterima <'$tgl_awal'  THEN jml_dibayar ELSE 0 END) AS kemarin ,
					SUM(CASE WHEN tgl_diterima ='$tgl_awal'  THEN jml_dibayar ELSE 0 END) AS hari_ini 
					FROM dt_pbb_bphtb a
					LEFT JOIN dt_detail_pbb_bphtb b ON a.no_sptpd = b.no_sptpd
					where year(tgl_diterima)='$tg[0]'
					GROUP BY kd_rek)b
					ON a.kd_rek=b.kd_rek
					UNION ALL
					SELECT * FROM(
					...
					WHERE LENGTH(kd_rek)='11' AND LEFT(kd_rek,5)='4.1.2' AND status_aktif='1')a
					LEFT JOIN (SELECT b.kode_rekening,
					SUM(CASE WHEN tanggal <'$tgl_awal'  THEN a.jumlah ELSE 0 END) AS kemarin ,
					SUM(CASE WHEN tanggal ='$tgl_awal'  THEN a.jumlah ELSE 0 END) AS hari_ini 
					FROM ssrd a
					LEFT JOIN ssrd_detail b ON a.no_ssrd = b.no_ssrd
					where year(tanggal)='$tg[0]'
					GROUP BY kode_rekening)b
		...
		/*$tot_bln_lalu 	= $query2->bln_lalu;
		$tot_bn_ini 	= $query2->bln_ini;*/
		$tot_kemarin 	= $query2->kemarin;
		$tot_hari_ini 	= $query2->hari_ini;
		$tot_jml 		= $tot_hari_ini+$tot_kemarin;
		//$tot_sisa 		= $tot_jml-$tot_target;
		/*$tot_sisa 		= $this->msistem->rp_minus($tot_jml-$tot_target);*/
		...
		foreach($query2->result() as $rs){
			...
			/*$bln_ini 		= $rs->bln_ini;
			$bln_lalu 		= $rs->bln_lalu;*/
			$hari_ini 		= $rs->hari_ini;
			$kemarin 		= $rs->kemarin;
			$jml			= $hari_ini+$kemarin;
			$tambah_kurang	= $jml-$target;
			$tambah_kurang1	= $this->msistem->rp_minus($tambah_kurang);
			...
			if(strlen($kd_rek)>=5){
				$muncul='<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:9px">
					...
					<td width="280" valign="center" align="left">'.$nm_rek.'</td>
					<td width="140" valign="center" align="right">'.number_format($target,2,",",".").'</td>
					<td width="140" valign="center" align="right">'.number_format($kemarin,2,",",".").'</td>
					<td width="140" valign="center" align="right">'.number_format($hari_ini,2,",",".").'</td>
					<td width="150" valign="center" align="right">'.number_format($jml,2,",",".").'</td>
				</tr>';
			}else{
				$muncul='<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:9px">
					<td width="70" valign="center" align="left"><strong>'.$kd_rek.'</strong></td>
					<td width="280" valign="center" align="left"><strong>'.$nm_rek.'</strong></td>
					<td width="140" valign="center" align="right"><strong>'.number_format($target,2,",",".").'</strong></td>
					<td width="140" valign="center" align="right"><strong>'.number_format($kemarin,2,",",".").'</strong></td>
					<td width="140" valign="center" align="right"><strong>'.number_format($hari_ini,2,",",".").'</strong></td>
					<td width="150" valign="center" align="right"><strong>'.number_format($jml,2,",",".").'</strong></td>
				</tr>';
			}
		...
		$report .='<table border="1">
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:10px">
					<td colspan="2" width="350" align="center"><strong>JUMLAH</strong></td>
					<td width="140" align="right"><strong>'.number_format($tot_target,2,",",".").'</strong></td>
					<td width="140" align="right"><strong>'.number_format($tot_kemarin,2,",",".").'</strong></td>
					<td width="140" align="right"><strong>'.number_format($tot_hari_ini,2,",",".").'</strong></td>
					<td width="150" align="right"><strong>'.number_format($tot_jml,2,",",".").'</strong></td>
				</tr>
				</table>';
