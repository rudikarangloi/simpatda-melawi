...
public function cariData($dasar="",$parameter="",$kata_kunci="") {	
	...
	if($dasar=='1'){	
		if($parameter=='1'){
			$qr = " and no_skrd like '%".$kata_kunci."%'";
		} else {
			$qr = " and skpd_pengelola like '%".$kata_kunci."%'";
		}
			$grid = new GridConnector($this->db->conn_id);
			$grid->dynamic_loading(100); 
			$grid->render_sql("SELECT id, no_skrd, skpd_pengelola, kriteria_htg, tahun, tgl_terima, no_sptrd, jumlah, jml_karcis, no_karcis1, no_karcis2, ket_skpd FROM skrd
							   WHERE status='0' $qr","id","no_skrd, skpd_pengelola, kriteria_htg, tahun, tgl_terima, no_sptrd, jumlah, jml_karcis, no_karcis1, no_karcis2, ket_skpd");
        ...
        function simpan(){            
			...
			if($_POST['dasar_setoran']==1){			     
                   ...
                   $upd = array(
				    ...
				    $this->db->update('skrd',$upd,array('no_skrd' => $_POST['no_skpd']));	
			} else {
				$upd = array(
					'status' => 1
				);
				$this->db->update('sptrd',$upd,array('no_sptpd' => $_POST['no_skpd']));
            ...
            $data = array(			
						...
						'kriteria_htg'     => $_POST['perhitungan'],
                        'tanggal'	    => strftime("%Y-%m-%d", strtotime($_POST['tanggal'])),
                        'no_skrd' 	    => $_POST['no_skpd'],
                        'no_sptrd'         => $_POST['sptrd'],
						'no_karcis1'   => $_POST['karcis1'],
						'no_karcis2'   => $_POST['karcis2'],
						'jumlah_karcis'   => $_POST['jml_karcis'],               
		...
		if(empty($_POST['edit'])){
			...
			$dataIns = array_merge($data,array('no_ssrd' =>  $no_sspd, 'created' => date('Y-m-d H:i:s'), 'user_created'=>$user));
			$result = $this->db->insert('ssrd', $dataIns);
			
			//$sql = $this->db->query("select ")->row();
		} else {
			$no_sspd = $this->input->post('sspd_1')."/".$this->input->post('sspd_2');
			$this->db->delete('ssrd_detail',array('no_ssrd' => $no_sspd));
			$dataUpdate = array_merge($data,array('user_modified' => $user,'modified' => date('Y-m-d H:i:s')));
			$result = $this->db->update('ssrd',$dataUpdate,array('no_ssrd' => $no_sspd));
		}
	...
	public function generateNo($thn="") {
		$sql = $this->db->query("select MAX(SUBSTRING_INDEX(no_ssrd,'/',1)) as max from ssrd where LEFT(tanggal,4) = '".$thn."'");
		$r = $sql->row();
		if($r==NULL){
	...
	public function smsgateway(){
		...
		$grid = new GridConnector($this->db->conn_id);
		/* $grid->dynamic_loading(100); */ 
		$grid->render_sql("SELECT id,no_ssrd,dasar_setoran,pembayaran,tanggal,no_skrd,skpd_pengelola,
kriteria_htg,no_sptrd,keterangan,jumlah,user_created,tahun_transaksi,ttd_ssrd,no_karcis1,no_karcis2,jumlah_karcis FROM ssrd ORDER BY no_ssrd DESC LIMIT 15"
,"id","id, no_ssrd, dasar_setoran, pembayaran, tanggal, no_skrd, skpd_pengelola, kriteria_htg, no_sptrd, keterangan, jumlah, user_created, tahun_transaksi, ttd_ssrd, no_karcis1,no_karcis2,jumlah_karcis");
	}
...
//$grid = new GridConnector($this->db->conn_id);
	...
	if($dasar==1){
		$sq = $this->db->query("SELECT kd_rek, nm_rek, dp, tarif, kenaikan, denda, bunga, kompensasi, jumlah FROM skrd_child WHERE skrd ='".$nomor."'");
...
function load_childsspd($sspd="") {
    ...
    bunga, jumlah,no_ssrd FROM ssrd_detail WHERE no_ssrd ='".$dasar."'");$i=1;
	foreach($sq->result() as $qr) {
	...
	public function delete(){
		...
		$this->db->update('skrd', $upd, array('no_skrd' => $this->input->post('skpd')));
		$this->db->delete('ssrd',array('no_ssrd' => $this->input->post('sspd')));
		$this->db->delete('ssrd_detail',array('no_ssrd' => $this->input->post('sspd')));
        //$No = $this->input->post('nomor');
        //$query= $this->db->query("update skpd set status='0' where no_skpd='$No'");
	...
	public function cetak(){
		...
		if($rek_detail->luas>=0.25&&$rek_detail->luas<=5.99){
			$nilai_sat = 500000;
