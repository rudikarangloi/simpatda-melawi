...
class pajaklain2 extends MY_Controller {
	...
	public function index() {
		...
		$data['jns_rek'] = $this->db->query("SELECT kd_rek,CONCAT(kd_rek, ' | ',nm_rek) AS nm_rek FROM master_rekening WHERE status_child = '12'");
		$data['jnsReklame'] = $this->db->query("SELECT * FROM ms_jenis_reklame WHERE masa_pajak ='Tahun'");
		$data['jnsReklame2'] = $this->db->query("SELECT * FROM ms_jenis_reklame WHERE masa_pajak !='Tahun'");
		...
		$data['skpd'] = $this->db->query("SELECT kd_skpd,CONCAT(kd_skpd, ' | ',nama_skpd) AS nama_skpd FROM master_skpd WHERE kd_skpd = '1.20.07'");
		//$data['skpd'] = $this->db->query("SELECT kd_skpd,nama_skpd FROM master_skpd");
		$data['tgl'] = date('d/m/Y');
        $data['tahun'] = $this->msistem->tahun();
		$this->load->view('data/pajaklain2',$data);
	}
	...
	public function crek_pajak() {
		...
		$e = $this->db->query("SELECT kd_rek,nm_rek FROM master_rekening WHERE kd_rek='".$id."'")->row();
		$result = $e->kd_rek.'|'.$e->nm_rek;
	...
	function pokok(){
		...
		$jns_rek2 = $this->input->post('gjenisrek2');
	...
	/* 	$sisi = $this->input->post('sisi');
		...
		$q1 = $this->db->query("SELECT kawasan,ns1,ns2,ns3,ns4,ns5,ns6,ns7,ns8 FROM master_lokasi WHERE kawasan ='".$kawasan."'")->row();
		$query2 = $this->db->query("SELECT id,jenis,luas_bidang FROM ms_jenis_reklame WHERE id ='".$jns_rek2."'")->row();
	/* 	$query3 = $this->db->query("SELECT sudut_pandang,skor,bobot FROM ms_sudut_pandang WHERE id ='".$sudut."'")->row();
		...
		$query5 = $this->db->query("SELECT lebar,skor FROM ms_lebar_jalan WHERE id ='".$lebar."'")->row();
	 */
		$nilai  = $q1->ns1;
		$nilai2 = $q1->ns2;
		$nilai3 = $q1->ns3;
		...
		$nilai_sewa = ($njop+$nilai_stgr)*$luas;
		$pokok_pajak = 0.1*$nilai_sewa;
		$nilai_pjk = $pokok_pajak*$lama_psg*$unit; 
...
public function cariDataWP($kata_kunci="",$parameter=""	) {	
		...
		$iddd = $_POST['buatidd'];	
            ...
            $data = array(			
						'skpd_pengelola' => $_POST['nama_perusahaan'],
		...
		if(empty($_POST['edit'])){
			...
			$result = $this->db->insert('dt_pbb_bphtb', $dataIns);
			...
			$result = $this->db->update('dt_pbb_bphtb',$dataUpdate,array('no_sptpd' => $no_sptpd));
			
		}
		
		$this->db->query("update dt_detail_pbb_bphtb set no_sptpd='".$no_sptpd."' where id_inputan='".$iddd."'");
		...
		$sql = $this->db->query("SELECT max_pjk FROM no_max WHERE id='1'");
		$r = $sql->row();
		$jml = $r->max_pjk + 1;
		if(strlen($jml)==1) {
			$jml = '00000000'.$jml;
		...
		$dtUpd = array('max_pjk' => $jml);
		$this->db->update('no_max', $dtUpd, array('id' => 1));
		
		return $jml.'/REG/'.substr($thn,2,2);
	}
	
	function reklame_detail(){
		$grid = new GridConnector($this->db->conn_id);
//		$grid->set_options("kawasan",array("1" => "AKTIF", "0"=>"TIDAK AKTIF"));
		$grid->render_table("dt_detail_pbb_bphtb","id","id_inputan,kd_rek,nm_rek,skpd_pengelola,penerimaan,no_sptpd");
	}
	
	function load_detail_edit($nos=""){
		$no = str_replace(".","/",$nos);
		$grid = new GridConnector($this->db->conn_id);
		$grid->render_sql("SELECT id_inputan,kd_rek,nm_rek,skpd_pengelola,penerimaan,no_sptpd FROM dt_detail_pbb_bphtb WHERE no_sptpd='".$no."'","id","id_inputan,kd_rek,nm_rek,skpd_pengelola,penerimaan,no_sptpd");
	}
        
        function load_data() {
		$grid = new GridConnector($this->db->conn_id);
		//$grid->dynamic_loading(100); 
		$grid->render_sql("SELECT a.no_sptpd,a.kd_skpd,a.skpd_pengelola,a.tgl_diterima,a.author,a.jml_dibayar,b.kd_rek,a.uraian FROM dt_pbb_bphtb a
						 INNER JOIN dt_detail_pbb_bphtb b ON a.no_sptpd=b.no_sptpd GROUP BY a.no_sptpd"
                        ,"id"
						,"no_sptpd,kd_skpd,skpd_pengelola,tgl_diterima,author,kd_rek,uraian,jml_dibayar");}
	
	public function hapus() {
		$sq = $this->input->post('sptpd_1')."/".$this->input->post('sptpd_2');
		
			$this->db->delete('dt_pbb_bphtb',array('no_sptpd' => $this->input->post('sptpd_1')."/".$this->input->post('sptpd_2')));
			$this->db->delete('dt_detail_pbb_bphtb',array('no_sptpd' => $sq));
		...
		$idc = "1";
		$no_urut = 0;
		$sql = $this->db->query("SELECT max_id FROM no_max WHERE id='".$idc."'")->row();		
		$maxx = $sql->max_id;
		if($maxx=="000000000") {
					$no_urut = "000000000";						
				} else {					
		...
		$this->db->query("update no_max set max_id='".$no_urut."' WHERE id='".$idc."'");		
...
LEFT JOIN identitas_perusahaan ON identitas_perusahaan.npwpd_perusahaan = sptpd_reklame.npwpd where sptpd_reklame.no_sptpd='".$_GET['sptpd']."'")->row();
	...
	function buatid(){
		...
		$id = "1";
		$no_urut = 0;
		$sql = $this->db->query("SELECT max_id FROM no_max WHERE id='1'")->row();		
		$maxx = $sql->max_id;
		if($maxx==NULL) {
					$noid = "REG."."000000001";						
				} else {					
		...
		$this->db->query("update no_max set max_id='".$no_urut."' where id='1'");	
	} 
...
<tr>
	...
	function cetak2(){
		$sql = $this->db->query("SELECT sptpd_reklame.id, sptpd_reklame.npwpd, sptpd_reklame.no_sptpd, view_perusahaan.npwpd_lama, sptpd_reklame.no_sptpd, view_perusahaan.nama_perusahaan, view_perusahaan.alamat_perusahaan,
 view_perusahaan.nama_pemilik, sptpd_reklame.cara_hitung, DATE_FORMAT(sptpd_reklame.tgl_diterima,'%d/%m/%Y') AS tgl_diterima, 
 sptpd_reklame.author, sptpd_reklame.sifat_reklame, DATE_FORMAT(sptpd_reklame.masa_pajak1,'%d/%m/%Y') AS masa_pajak1,
  DATE_FORMAT(sptpd_reklame.masa_pajak2,'%d/%m/%Y') AS masa_pajak2, 
 sptpd_reklame.jml_bayar, sptpd_reklame.waktu_pasang, sptpd_reklame.kawasan_pasang, 
...
view_perusahaan ON sptpd_reklame.npwpd=view_perusahaan.npwpd_perusahaan where sptpd_reklame.no_sptpd='".$_GET['sptpd']."'")->row();
		...
		$report = '';
			...
			$nilai_pjk2 = $nilai_pjk*$rek_detail->unit; 
...
<tr>
	<td colspan="3" height="10">&nbsp;</td>
							...
							<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold; font-size:9px">
								...
								<td width="10" align="center">:</td>
								<td width="340">'.$rek_detail->luas.' M<sup>2</sup></td>
							</tr>						
							<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold; font-size:9px">
								<td width="220">&nbsp;&nbsp;4. Tempat Pemasagan Reklame</td>
								<td width="10" align="center">:</td>
								<td width="340">'.$rek_detail->lokasi.'</td>
							</tr>
							<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold; font-size:9px">
								<td width="220">&nbsp;&nbsp;5. Lama Pemasangan/Jangka waktu</td>
								<td width="10" align="center">:</td>
								...
								<td width="340">
									<table border=1>
										<tr>
											...
											<td width="190" align="right">
												<b>'.number_format($nilai_stgr,2,",",".").'</b>
											</td>
										</tr>
									</table>
								</td>
									...
									<table border=1>
										<tr>
											<td width="340" align="left">
												<b>&nbsp;&nbsp;10 % x (NJOP + Nilai Strategis) x Luas x Lama Pemasangan</b>
											</td>
										</tr>
										<tr>
											...
											<td width="225" align="right">
												<b>('.number_format($njop,0,",",".").' + '.number_format($nilai_stgr,0,",",".").') x '.$rek_detail->luas.' x '.$lama_psg.'</b>
											</td>
										</tr>
										<tr>
											...
											<td width="265" align="right">
												<b>'.number_format($nilai_pjk,0,",",".").'</b>
											</td>
										</tr>
										<tr>
											<td width="75" align="center">
												<b>'.$rek_detail->unit.'  Buah</b>
											</td>
											<td width="20" align="center">
												<b>x</b>
											</td>
										...
										<tr>
											...
											<td width="265" align="center">
												<b>'.number_format($nilai_pjk2,0,",",".").'</b>
											</td>
										</tr>
									</table>
								</td>
