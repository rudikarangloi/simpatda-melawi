...
WHERE a.status='0' $qr GROUP BY a.no_skpd ","no_sptpd","no_skpd, npwpd, masa_pajak1, masa_pajak2, tahun, jumlah, nama_pemilik, alamat_pemilik, nama_perusahaan, alamat_perusahaan, no_sptpd, nota_hitung, tgl, ms_1, ms_2, denda, ket_skpd");
	...
	public function smsgateway(){
		...
		$query = $this->db->query('select hp from view_perusahaan where npwpd_perusahaan="'.$npwpd.'"');
		//$soq = $sqe->hp;
			if($query==NULL){
				//$ss = '083893121648';
				$ss = $query;
			...
			if($kode=='REK'){
				...
				foreach($f->result() as $ch) {
					...
					$report .= '<tr id='.$i.'>
					  ...
					  <td width="80" height="15" align="center" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">'.$ch->kd_rek.'</td>
					  <td width="160" height="15" align="center" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">'.$ch->nm_rek.'</td>
					  <td width="187" height="15" align="center" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">'.$ch->kawasan.' x '.$ch->luas.'m<sup>2</sup> x '.$ch->unit.'bh x '.number_format($ch->tarif,2,',','.').' x '.$ch->trf.'%</td>
