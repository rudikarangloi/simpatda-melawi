...
class pajaklain extends MY_Controller {
	...
	public function index() {
		...
		$data['jns_rek'] = $this->db->query("SELECT kd_rek,CONCAT(kd_rek, ' | ',nm_rek) AS nm_rek FROM master_rekening WHERE jns_pajak ='4.2' ORDER BY kd_rek");
		...
		$this->load->view('data/pajaklain',$data);
	}
...
public function cariDataWP($kata_kunci="",$parameter=""	) {	
		...
		if(empty($_POST['edit'])){
			...
			$result = $this->db->insert('dt_pajaklain', $dataIns);
			...
			$result = $this->db->update('dt_pajaklain',$dataUpdate,array('no_sptpd' => $no_sptpd));
			
		}
		
		$this->db->query("update dt_detail_pajaklain set no_sptpd='".$no_sptpd."' where id_inputan='".$iddd."'");
...
//		$grid->set_options("kawasan",array("1" => "AKTIF", "0"=>"TIDAK AKTIF"));
		$grid->render_table("dt_detail_pajaklain","id","id_inputan,kd_rek,nm_rek,skpd_pengelola,penerimaan,no_sptpd");
	}
	
	function load_detail_edit($nos=""){
		$no = str_replace(".","/",$nos);
		$grid = new GridConnector($this->db->conn_id);
		$grid->render_sql("SELECT id_inputan,kd_rek,nm_rek,skpd_pengelola,penerimaan,no_sptpd FROM dt_detail_pajaklain WHERE no_sptpd='".$no."'","id","id_inputan,kd_rek,nm_rek,skpd_pengelola,penerimaan,no_sptpd");
	}
        
        function load_data() {
		$grid = new GridConnector($this->db->conn_id);
		//$grid->dynamic_loading(100); 
		$grid->render_sql("SELECT a.no_sptpd,a.kd_skpd,a.skpd_pengelola,a.tgl_diterima,a.author,a.jml_dibayar,b.kd_rek,a.uraian FROM dt_pajaklain a
						 INNER JOIN dt_detail_pajaklain b ON a.no_sptpd=b.no_sptpd GROUP BY a.no_sptpd"
                        ,"id"
	...
	public function hapus() {
		$sq = $this->input->post('sptpd_1')."/".$this->input->post('sptpd_2');
			...
			$this->db->delete('dt_pajaklain',array('no_sptpd' => $this->input->post('sptpd_1')."/".$this->input->post('sptpd_2')));
			$this->db->delete('dt_detail_pajaklain',array('no_sptpd' => $sq));
