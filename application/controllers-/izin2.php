...
class izin extends MY_Controller {
	...
	public function ckel() {
		...
		$e = $this->db->query("select nama_kecamatan from view_kelurahan where id='".$id."'")->row();
	...
	public function ckec() {
		$id = $this->input->post('kelurahan');
		$e = $this->db->query("select kode_kelurahan, kode_kecamatan, nama_kecamatan from view_kelurahan where kode_kelurahan='".$id."'")->row();
	...
	public function load_pemilik($op="",$nilai="") {
		...
		$qr = $this->db->query("select id,npwpd_pemilik,jns_identitas,kewarganegaraan,no_identitas,nama_pemilik,alamat_pemilik,jalan,rt,rw,email,desa_kel2,kecamatan,kabupaten,tempat_lahir,DATE_FORMAT(tanggal_lahir,'%d/%m/%Y') as tanggal_lahir,jenis_kelamin,lokasi,hp from identitas_pemilik where status='0' $sqr");
	...
	public function search_pemilik($op="",$nilai="") {
		...
		if($op=='1'){
			$sqr = "where npwpd_pemilik like '%".$nilai."%'";
		} else if($op=='2'){
			$sqr = "where nama_pemilik like '%".$nilai."%'";
		} else if($op=='3'){
			$sqr = "where alamat_pemilik like '%".$nilai."%'";
		} else if($op=='4'){
			$sqr = "where no_identitas like '%".$nilai."%'";
		} else if($op=='5'){
			$sqr = "where email like '%".$nilai."%'";
		} else if($op=='6'){
			$sqr = "where kewarganegaraan like '%".$nilai."%'";
		}
		$qr = $this->db->query("select id,npwpd_pemilik,jns_identitas,kewarganegaraan,no_identitas,nama_pemilik,alamat_pemilik,jalan,rt,rw,email,desa_kel2,kecamatan,kabupaten,tempat_lahir,DATE_FORMAT(tanggal_lahir,'%d/%m/%Y') as tanggal_lahir,jenis_kelamin,lokasi,hp from identitas_pemilik $sqr");
	...
	public function perusahaan() {
		...
		$data['nm_modul'] = $s->nm_modul;
		$data['usaha']=$this->db->query("select kd_jns, nm_usaha from jenis_usaha order by id_usaha asc");
	...
	public function load_perusahaan($op="",$nilai="") {
		...
		$qr = $this->db->query("select a.id, a.npwpd_perusahaan, a.nama_perusahaan, a.alamat_perusahaan, a.jalan, a.rt, a.rw, a.email, a.kelurahan, d.nama_kelurahan, a.kecamatan, c.nama_kecamatan, a.kabupaten, a.telp, a.kodepos, b.npwpd_pemilik, b.nama_pemilik, b.alamat_pemilik, b.jalan as jalan_p, b.rt as rt_p, b.rw as rw_p, b.email as email_p, b.lokasi, b.desa_kel2, b.kecamatan as kecamatan_p, b.kabupaten as kabupaten_p, b.hp, b.kodepos, a.jenis_usaha, a.status_usaha, a.kategori, a.jml_karyawan, a.ukuran_tempat, a.surat_izin, a.no_surat, DATE_FORMAT(a.tgl_surat,'%d/%m/%Y') as tgl_surat, a.jenis_pajak, a.jenis_pajak_detail, DATE_FORMAT(a.tgl_daftar,'%d/%m/%Y') as tgl_daftar from identitas_perusahaan a inner join identitas_pemilik b on a.npwpd_pemilik = b.npwpd_pemilik inner join kecamatan c on a.kecamatan = c.kode_kecamatan inner join kelurahan d on a.kelurahan = d.kode_kelurahan $cu");
	...
	public function data_perusahaan() {
		$grid = new GridConnector($this->db->conn_id);
		$grid->render_sql("select a.id, a.npwpd_perusahaan, a.nama_perusahaan, a.alamat_perusahaan, a.jalan, a.rt, a.rw, a.email, a.kelurahan, d.nama_kelurahan, a.kecamatan, c.nama_kecamatan, a.kabupaten, a.telp, a.kodepos, b.npwpd_pemilik, b.nama_pemilik, b.alamat_pemilik, b.jalan as jalan_p, b.rt as rt_p, b.rw as rw_p, b.email as email_p, b.lokasi, b.desa_kel2, b.kecamatan as kecamatan_p, b.kabupaten as kabupaten_p, b.hp, b.kodepos, a.jenis_usaha, a.status_usaha, a.kategori, a.jml_karyawan, a.ukuran_tempat, a.surat_izin, a.no_surat, DATE_FORMAT(a.tgl_surat,'%d/%m/%Y') as tgl_surat, a.jenis_pajak, a.jenis_pajak_detail, DATE_FORMAT(a.tgl_daftar,'%d/%m/%Y') as tgl_daftar from identitas_perusahaan a inner join identitas_pemilik b on a.npwpd_pemilik = b.npwpd_pemilik inner join kecamatan c on a.kecamatan = c.kode_kecamatan inner join kelurahan d on a.kelurahan = d.kode_kelurahan","id","id, npwpd_perusahaan, nama_perusahaan, alamat_perusahaan, jalan, rt, rw, email, kelurahan, nama_kelurahan, kecamatan, nama_kecamatan, kabupaten, telp, kodepos, npwpd_pemilik, nama_pemilik, alamat_pemilik, jalan_p, rt_p, rw_p, email_p, lokasi, desa_kel2, kecamatan_p, kabupaten_p, hp, kodepos, jenis_usaha, status_usaha, kategori, jml_karyawan, ukuran_tempat, surat_izin, no_surat, tgl_surat, jenis_pajak, jenis_pajak_detail, tgl_daftar");	
	}
	...
	public function simpan_perusahaan() {
		...
		*/
		...
		if($id_perusahaan==NULL){
			//kode kelurahan
			$idkel = $this->input->post('kelurahan');
			$skel = $this->db->query("select kode_kelurahan,kode_kecamatan from view_kelurahan where kode_kelurahan='".$idkel."'")->row();
			$kdkel = $skel->kode_kelurahan;
			/*if(strlen($kdkel)==1){
				$ckel = '0'.$kdkel;
			} else if(strlen($kdkel)==2){
			*/	$ckel = $kdkel;
			...
			$q = $this->db->query("select max(npwpd_perusahaan) as nos from identitas_perusahaan where jenis_pajak ='".$pajak."'")->row();
			$nos = $q->nos;
			
			if($nos==NULL) {
				$id_perusahaan = $pajak.".000001.".$ckec.".".$ckel;
			} else {
				$cek = explode(".",$nos);
				$no = $cek[3] + 1;
				...
				$id_perusahaan = $pajak.".".$no_urut.".".$ckec.".".$ckel;
			}
		}
	...
	public function cetak_perusahaan() {
		...
		$data = $this->db->query("select a.npwpd_perusahaan as npwpd_perusahaan, a.nama_perusahaan as nama_perusahaan, a.alamat_perusahaan as alamat_perusahaan, DATE_FORMAT(a.terdaftar,'%d/%m/%Y') as tgl, b.nama_sptpd as nm_usaha from identitas_perusahaan a inner join master_sptpd b on a.jenis_pajak=b.id where a.npwpd_perusahaan='".$_GET['npwpd']."'")->row();
	...
	public function data_pengukuhan() {
		$grid = new GridConnector($this->db->conn_id);
		$grid->render_sql("select a.id, a.no_ukuh, a.npwpd, b.nama_perusahaan, b.alamat_perusahaan, a.kewajiban from pengukuhan a left join identitas_perusahaan b on a.npwpd = b.npwpd_perusahaan","id","id, no_ukuh, npwpd, nama_perusahaan, alamat_perusahaan, kewajiban");	
	}
...
identitas_perusahaan.alamat_perusahaan, pengukuhan.kewajiban FROM identitas_perusahaan INNER JOIN pengukuhan ON pengukuhan.npwpd = identitas_perusahaan.npwpd_perusahaan where pengukuhan.npwpd ='".$_GET['npwpd']."'")->row();
		$npwpd       = $data->npwpd;
