...
public function cariData($dasar="",$parameter="",$kata_kunci="") {	
	...
	if($dasar=='1'){	
		if($parameter=='1'){
			...
			$grid->render_sql("select a.no_skpd, a.npwpd, a.masa_pajak1, a.masa_pajak2, a.tahun, a.jumlah, b.nama_pemilik, b.alamat_pemilik, b.nama_perusahaan, b.alamat_perusahaan, a.no_sptpd, a.nota_hitung, DATE_FORMAT(a.tgl_jth_tempo,'%d/%m/%Y') as tgl from skpd a left join view_perusahaan b on a.npwpd=b.npwpd_perusahaan where a.status ='0' $qr","no_sptpd","no_skpd, npwpd, masa_pajak1, masa_pajak2, tahun, jumlah, nama_pemilik, alamat_pemilik, nama_perusahaan, alamat_perusahaan, no_sptpd, nota_hitung, tgl");
	...
	public function generateNo($thn="") {
		$sql = $this->db->query("select MAX(SUBSTRING_INDEX(no_sspd,'/',1)) as max from sspd where tahun_transaksi = '".$thn."'");
		$r = $sql->row();
		if($r==NULL){
...
FROM
view_perusahaan
INNER JOIN sspd ON sspd.npwpd = view_perusahaan.npwpd_perusahaan"
,"id"
                        ,"id, no_sspd, dasar_setoran, pembayaran, tanggal, nomor, npwpd, nama_perusahaan, alamat_perusahaan, nama_pemilik, alamat_pemilik, masa_pajak1, masa_pajak2, tahun_pajak, keterangan, ketetapan, setoran, persen, created, modified, user_created, user_modified, nota_hitung, denda");
	}
...
//$grid = new GridConnector($this->db->conn_id);
	...
	if($dasar==1){
		$sq = $this->db->query("SELECT kd_rek, nm_rek, dp, tarif, jumlah FROM skpd_child WHERE skpd ='".$nomor."'");
...
//        $query= $this->db->query(" update sptpd set status='0' where no_sptpd='$No' ");
	...
	public function cetak(){
		...
		$data = $this->db->query("select a.dasar_setoran, a.nomor, a.no_sspd, b.nama_pemilik, b.alamat_pemilik, b.nama_perusahaan, b.alamat_perusahaan, a.masa_pajak1, a.masa_pajak2, a.tahun_pajak, DATE_FORMAT(a.tanggal,'%d/%m/%Y') as tgl, a.npwpd, a.ketetapan, a.setoran, a.persen, a.denda, a.kode_pajak from sspd a left join view_perusahaan b on a.npwpd=b.npwpd_perusahaan where a.no_sspd='".$_GET['sspd']."'")->row();
		$no 				= $data->no_sspd;
		$sptpd 			= $data->nomor;
		$npwpd 				= $data->npwpd;
		...
		$report .=
			'<table border="1">
				<tr>
					...
					<td width="80" height="20" align="center">Kode Rekening</td>
					<td width="160" height="20" align="center">Nama Rekening</td>
					<td width="187" height="20" align="center">Uraian</td>
			...
			if($kode=='REK'){
				...
				foreach($f->result() as $ch) {
					$report .= '<tr id='.$i.'>
					  ...
					  <td width="80" height="20" align="center">'.$ch->kd_rek.'</td>
					  <td width="160" height="20" align="center">'.$ch->nm_rek.'</td>
					  <td width="187" height="20" align="center">'.$ch->kawasan.' x '.$ch->luas.'m<sup>3</sup> x '.$ch->unit.'bh x '.number_format($ch->tarif,2,',','.').' x '.$ch->trf.'%</td>
				...
				foreach($child->result() as $ch) {
					$report .= '<tr id='.$i.'>
					  <td width="25" height="20" align="center">'.$i.'</td>
					  <td width="80" height="20" align="center">'.$ch->kode_rekening.'</td>
					  <td width="160" height="20" align="center">'.$ch->nama_rekening.'</td>
					  <td width="187" height="20" align="center">'.number_format($ch->dp,2,',','.').' x '.$ch->tarif.'%</td>
		...
		$report .=
				'<tr>
					  <td colspan="3" height="20" width="452" align="right">Jumlah  Pajak Terhutang&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
					  <td width="120" height="20" align="right">'.number_format($data->ketetapan,2,',','.').'&nbsp;&nbsp;</td>
				</tr>
				<tr>
					  <td colspan="3" height="20" width="452" align="right">Denda Pajak&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
					  <td width="120" height="20" align="right">'.number_format($data->denda,2,',','.').'&nbsp;&nbsp;</td>
				</tr>
				<tr>
					  <td colspan="3" height="20" width="452" align="right">Jumlah Setoran Pajak&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		...
		$report .=
			'<table border="1">
				<tr>
					...
					<table border="0">
						<tr>
							<td width="450" height="20" colspan="3">&nbsp;&nbsp;  - Bendahara Penerimaan / Bendahara Penerimaan Pembantu Dinas Pendapatan Kota Jambi<br/>&nbsp;    Telah menerima Uang sebesar Rp. '.number_format($data->setoran,2,',','.').'</td>							
						</tr>
						<tr>
							<td width="80" height="20">&nbsp;&nbsp;  - Dengan huruf</td>
							<td width="10" align="center">:</td>
			...
			'<table border="1">
				...
				<table border="0">
					...
					<tr>
						...
						<td width="190" align="center">&nbsp;</td>
						<td width="190" align="center">Jambi, '.$tanggal.'</td>
					</tr>
					<tr>
						<td width="190" align="center">'.$bagian3.'</td>
						<td width="190" align="center">Bendahara Penerimaan / Bendahara Penerimaan Pembantu</td>
						<td width="190" align="center">Pembayar / Penyetor</td>
					</tr>
					<tr>
						<td width="190" align="center">&nbsp;</td>
						<td width="190" align="center">&nbsp;</td>
					...
					<tr>
						<td width="190" align="center"><u> '.$nama3.' </u></td>
						<td width="190" align="center"><u> DINA HERMINA, SE </u></td>
						<td width="190" align="center"><u> '.$nama.' </u></td>
					</tr>
					<tr>
						<td width="190" align="center">NIP. '.$nip3.'</td>
						<td width="190" align="center">NIP. 19691225 199303 2 006</td>
						<td width="190" align="center"></td>
					</tr>
					<tr>
						<td colspan="3">&nbsp;</td>
			...
			<table border="1">
				<tr>
					...
					<table border="0">						
						<tr>
							<td width="10" align="center"></td>
							<td width="50" align="center"><font size="8">Lembar Asli</font></td>
							<td width="10" align="center">:</td>
							<td width="450" align="justify"><font size="8">Untuk Pembayaran / Penyetoran / Pihak Ketiga</font></td>
						</tr>						
						<tr>
							<td width="10" align="center"></td>
							<td width="50" align="center"><font size="8">Salinan 1</font></td>
							<td width="10" align="center">:</td>
							<td width="450" align="justify"><font size="8">Untuk Bendahara Penerimaan / Bendahara Pembantu</font></td>
						</tr>
						<tr>
							<td width="10" align="center"></td>
							<td width="50" align="center"><font size="8">Salinan 2</font></td>
							<td width="10" align="center">:</td>
							<td width="450" align="justify"><font size="8">Untuk Kabid Akuntansi dan Pelaporan</font></td>
						</tr>
						<tr>
							<td width="10" align="center"></td>
							<td width="50" align="center"><font size="8">Salinan 3</font></td>
							<td width="10" align="center">:</td>
							<td width="450" align="justify"><font size="8">Untuk Kabid Pendataan, Penetapan dan Penagihan</font></td>
						</tr>												
					</table>
					</td>
				</tr>
