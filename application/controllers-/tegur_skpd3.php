...
class tegur_skpd3 extends MY_Controller {
	...
	public function index() {
		...
		$this->load->view('peneguran/tegur_skpd4',$data);
	}
...
INNER JOIN stpd ON stpd.npwpd = identitas_perusahaan.npwpd_perusahaan $qr","id","no_stpd, no_nota, npwpd, nama_perusahaan, alamat_perusahaan, tanggal_tempo,jumlah");
	...
	public function simpan(){
		...
		if($this->input->post('id')==0){
			...
			$this->db->insert('surat_tegur3',$dataIns);
			//get id
			$query = $this->db->query("SELECT id_surat_tegur FROM surat_tegur3 WHERE no_teguran = '$no_tegur' AND tahun = '".date('Y')."'");
			$row = $query->row();
			...
			$this->db->update('surat_tegur3', $dataUpd, array('id_surat_tegur' => $this->input->post('id')));
	...
	public function generateNo($thn=""){
		$sql = $this->db->query("select MAX(no_teguran) as max from surat_tegur3 where tahun = '".$thn."'")->row();
		$r = $sql->max;
		if($r==NULL){
			$no = '000000001';
		} else {
...
a.kecamatan, c.nama_kecamatan, a.kabupaten, a.telp, a.kodepos, b.npwpd_pemilik, b.nama_pemilik, b.alamat_pemilik, b.jalan AS jalan_p,
 b.rt AS rt_p, b.rw AS rw_p, b.email AS email_p, b.lokasi, b.desa_kel2, b.kecamatan AS kecamatan_p, b.kabupaten AS kabupaten_p, b.hp,
  ...
  IFNULL(c.id,'00') AS idx, e.auto AS idauto,SUBSTRING(a.npwpd_perusahaan,9,2) AS rek,d.pemilik_pimpinan,d.tunggakan, DATE_FORMAT(d.tgl_surat,'%d/%m/%Y') AS tgl, d.no_tegur_1
  ...
  LEFT JOIN surat_tegur3 d ON d.npwpd = a.npwpd_perusahaan 
  ...
  GROUP BY a.id")->row();
	...
	<tr>
		...
		<tr>
					...
					<td width="235"><div align="left"><font size="16" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif"><u><b>SURAT TEGURAN II</b></u></font><br><font size="11" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif,Helvetica Neue,Helvetica,sans-serif">&nbsp;&nbsp;Nomor   : 973/         /BPPRD.</br></font></div></td>
				</tr>
		<tr>
			<td>
			</td>
		...
		<tr style="line-height:30%;">
		  ...
		  <tr>
			<td width="40"></td>
			<td width="489" colspan="3" align="justify" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif,Helvetica Neue,Helvetica,sans-serif"><font size="11"><font color="white">====</font>'.$cetak.'</font></td>
		</tr>
		...
		$pdf->lastPage();
		$pdf->Output('Surat_Tegur_II'.'.pdf', 'I');
	}
	...
	public function delete() {
		...
		if ($id=="" || $npwpd==""){
			...
			$sql = $this->db->query("DELETE FROM surat_tegur3 where npwpd='".$npwpd."' and id_surat_tegur='".$id."'");
			if($sql){
				$result = 1;
			}else{
				$result = 0;
	...
	public function data() {
		$grid = new GridConnector($this->db->conn_id);
		$grid->dynamic_loading(100); 
		$grid->render_sql("SELECT a.id_surat_tegur, a.no_teguran, a.no_tegur_1 ,a.npwpd, DATE_FORMAT(a.tgl_surat,'%d/%m/%Y') AS tgl_surat, DATE_FORMAT(a.tgl_surat2,'%d/%m/%Y') AS tgl_surat2, b.nama_perusahaan, b.alamat_perusahaan, a.kewajiban,a.pemilik_pimpinan,a.tunggakan ,c.nama_sptpd AS jenis_usaha FROM surat_tegur3 a 
LEFT JOIN identitas_perusahaan b ON a.npwpd = b.npwpd_perusahaan
LEFT JOIN master_sptpd c ON c.id = b.jenis_usaha GROUP BY a.id_surat_tegur","id","id_surat_tegur, no_teguran, no_tegur_1,tgl_surat,tgl_surat2, npwpd, nama_perusahaan, alamat_perusahaan, kewajiban,pemilik_pimpinan,tunggakan, jenis_usaha");	
	}
	//-----fadil
