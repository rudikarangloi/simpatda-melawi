...
a.kecamatan, c.nama_kecamatan, a.kabupaten, a.telp, a.kodepos, b.npwpd_pemilik, b.nama_pemilik, b.alamat_pemilik, b.jalan AS jalan_p,
 b.rt AS rt_p, b.rw AS rw_p, b.email AS email_p, b.lokasi, b.desa_kel2, b.kecamatan AS kecamatan_p, b.kabupaten AS kabupaten_p, b.hp,
  ...
  IFNULL(c.id,'00') AS idx, e.auto AS idauto, a.insidentil AS insidentil
		...
		if($id_perusahaan==NULL){
			...
			*/	$ckel = $kdkel;
			...
			$nos = $q->nos;
                        ...
                        $update_master_rinci = true;
                        $update_no_max_npwpd = true;
                        if($insidentil == "1"){
                            //cari npwpd lewat
                            $i = 1;
                            $qr = $this->db->query("SELECT npwpd_perusahaan FROM identitas_perusahaan ORDER BY SUBSTR(npwpd_perusahaan,3,5)");	
                            foreach($qr->result() as $rs) {
                                $npwpd_perusahaan = $rs->npwpd_perusahaan;
                                $npwpd_perusahaan = explode(".",$npwpd_perusahaan);
                                $npwpd_perusahaan = (int)$npwpd_perusahaan[1];
                                if($npwpd_perusahaan == $i){
                                    $i++;
                                }else{
                                    $i--;
                                    break;
			...
			//bowo
                        ...
                        if($insidentil == "1"){
                            //cari nomor rekening lewat
                            $i = 1;
                            $qr = $this->db->query("SELECT npwpd_perusahaan FROM identitas_perusahaan WHERE SUBSTR(npwpd_perusahaan,1,1) = '$Gol' AND SUBSTR(npwpd_perusahaan,9,2) = '$nkode' ORDER BY SUBSTR(npwpd_perusahaan,12,3)");	
                            foreach($qr->result() as $rs) {
                                $npwpd_perusahaan = $rs->npwpd_perusahaan;
                                $npwpd_perusahaan = explode(".",$npwpd_perusahaan);
                                $npwpd_perusahaan = (int)$npwpd_perusahaan[3];
                                if($npwpd_perusahaan == $i){
                                    $i++;
                                }else{
                                    $i--;
			...
			if($update_no_max_npwpd) $this->db->update('no_max_npwpd', $maxUpd, array('kd_jns' => $pajak));	

			//bowo
			$maxUpd1 = array (
				'max' => $nor
			);
			if($update_master_rinci) $this->db->update('master_rinci', $maxUpd1, array('auto' => $reke));	
