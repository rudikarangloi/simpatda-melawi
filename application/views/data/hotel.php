...
<script language="javascript">
	...
	function disableData1() {
		...
		document.frmSKPDHotel.txtblnmasapajak1.disabled = false;
	...
	function cek(){
	   if(document.frmSKPDHotel.SPTR.checked==true) {
		...
		var postCek =
            ...
            "&jnsSPT=" + jnsSPT;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/hotel/ricek', postCek, resPOST);
	}
...
<div id="a_tabbar" style="width:1000px; height:530px; margin-left:30px;"></div>
  <div id="C1" style="background-color: #B3D9F0">
    <form name="frmSKPDHotel" id="frmSKPDHotel" action="<?php echo site_url(); ?>/hotel/cetak" method="post" onSubmit="popupform(this, 'iui')" >
    <br />
   	  <table width="787" border="0">
      		<tr>
        		<td width="198" style="padding-left:15px;">No. SPTPD Hotel</td>
            ...
            <tr>
                ...
                <input type="text" name="npwpd" id="npwpd" size="32" style="text-transform:uppercase; background-color: #FFCC99;" />
                <input type="button" onclick="opens()" value="Cari" id="cari1" name="cari1" style="padding-left:20px; padding-right:20px" disabled/>
                </td>
    		</tr>
            <tr>
              	<td style="padding-left:15px;"> Nama Usaha</td>
                <td><input type="text" name="nama_perusahaan" id="nama_perusahaan" size="45" style="background-color:#FFFFCC;" disabled/></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td colspan="2"><input type="checkbox" name="kasda" id="kasda" onchange="set_tanggal_kasda();" />
                Kasda (Transfer Bank)</td>
            </tr>
            <tr>
              	<td style="padding-left:15px;">Tanggal Diterima</td>
            ...
            <tr>
              ...
              <option value="<?php echo $userPetugas; ?>" selected="selected"><?php echo $namaPetugas; ?></option>
           	...
           	<tr>
              	<td style="padding-left:15px;">Golongan Hotel</td>
            ...
            <tr>
                ...
                <td><input style="text-align:right" type="text" name="tarif" id="tarif" size="20" disabled/>&nbsp;%
	...
	<tr>       	
        ...
        <input type="button" value="SPTPD Kosong" onclick="kosong()" name="null" id="null" /></td>
	</tr>
</table>
</form>
<br />
...
<script language="javascript">
	...
	function tarif_pajak(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/hotel/tarif', postGol, respJns);			
	}
        
        function get_bln_blm_bayar(){
            var npwpd = "npwpd="+document.frmSKPDHotel.npwpd.value;
            dhtmlxAjax.post(base_url+'index.php/hotel/get_bayar_terakhir', npwpd, respBulan);
        }
	...
	function hitung(jumlahDP){
		/*if(document.frmSKPDHotel.sen1.checked == true){
			...
			total = (nilai_pot*document.frmSKPDHotel.tarif.value)/110;
		...
		if(document.frmSKPDHotel.sen2.checked == true){
			...
			total = (nilai_pot*document.frmSKPDHotel.tarif.value)/100;
			document.frmSKPDHotel.setoran.value = format_number(total);
		}
	}
	...
	function lihat3() {
		...
		gridx.clearAll();
		gridx.loadXML("<?php echo site_url(); ?>/hotel/load_perusahaan4/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	grid.setHeader("id, NPWPD, No SPTPD, Nama Usaha, Alamat, Jenis, KET Insidentil, Cara Hitung, Tanggal Terima, Petugas Input, Gol Hotel, Masa Pajak 1, Masa Pajak 2, Omset, Jumlah Bayar, Tarif, ket_pajak, ket_inap");
	...
	grid.setInitWidths("50,170,150,150,100,100,100,100,100,100,100,100,100,100,100,100,100,100");
	grid.setColAlign("center,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left");
	grid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ron,ron,ron,ro,ro");
	grid.setColSorting("str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str");
	...
	grid.loadXML("<?php echo site_url(); ?>/hotel/data");
	grid.init();
        baru();
	...
	function selectedOpenData(id) {
		...
		document.frmSKPDHotel.npwpd.value	= grid.cells(id,1).getValue();
		document.frmSKPDHotel.sptpd.value = grid.cells(id,2).getValue();
		reg = grid.cells(id,2).getValue();
		arr = reg.split("/");
		document.frmSKPDHotel.urut.value = arr[0];
		document.frmSKPDHotel.hotel.value = arr[1]+'/'+arr[2];
		document.frmSKPDHotel.nama_perusahaan.value	= grid.cells(id,3).getValue();
		document.frmSKPDHotel.alamat_perusahaan.value	= grid.cells(id,4).getValue();
		
		if(grid.cells(id,5).getValue() == 'REGULAR') {
			document.frmSKPDHotel.SPTR.checked = true;
		} else if(grid.cells(id,5).getValue()=='INSIDENTIL') {
			document.frmSKPDHotel.SPTI.checked = true;
		}/* else if(grid.cells(id,5).getValue()=='TUNGGAKAN') {
			document.frmSKPDHotel.SPTT.checked = true;
		}*/
		document.frmSKPDHotel.ket_insidentil.value = grid.cells(id,6).getValue();
		document.frmSKPDHotel.cara.value 	= grid.cells(id,7).getValue();
		document.frmSKPDHotel.tgl.value 	 	= grid.cells(id,8).getValue();
		document.frmSKPDHotel.petugas.value 	 	= grid.cells(id,9).getValue();
		document.frmSKPDHotel.gol.value 	 	= grid.cells(id,10).getValue();
		tgl1 = grid.cells(id,11).getValue();
		arr1 = tgl1.split('/');
		document.frmSKPDHotel.txttglmasapajak1.value = grid.cells(id,11).getValue();
		
		tgl2 = grid.cells(id,12).getValue();
		arr2 = tgl2.split('/');
		document.frmSKPDHotel.txttglmasapajak2.value = grid.cells(id,12).getValue();
		
		document.frmSKPDHotel.txtblnmasapajak1.value = arr1[1];
		document.frmSKPDHotel.txtblnmasapajak2.value = arr2[1];
		document.frmSKPDHotel.txtthnmasapajak.value = arr1[2];
		document.frmSKPDHotel.jumlah.value 	= format_number(grid.cells(id,13).getValue());
		document.frmSKPDHotel.setoran.value 	= format_number(grid.cells(id,14).getValue());
		document.frmSKPDHotel.tarif.value = grid.cells(id,15).getValue();
		/*if(grid.cells(id,16).getValue() == 'TERMASUK PAJAK') {
			document.frmSKPDHotel.sen1.checked = true;
		} else if(grid.cells(id,16).getValue()=='BELUM TERMASUK PAJAK') {
			document.frmSKPDHotel.sen2.checked = true;
		}*/
		if(grid.cells(id,16).getValue()=='BELUM TERMASUK PAJAK') {
			document.frmSKPDHotel.sen2.checked = true;
		}
		document.frmSKPDHotel.ket_inap.value = grid.cells(id,17).getValue();
		statusEnding();
	}
		...
		if (name!=null){
			if(name=='dispenda'){
				...
				if(confrm) {
					...
					statusLoading();
					dhtmlxAjax.post(base_url+'index.php/hotel/delete', postStr, responeDel);	
				}
			} else {
				alert("Password anda salah");
			}
