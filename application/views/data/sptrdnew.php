...
<script language="javascript">
	...
	$(function(){
			$('#subjenis').combogrid({  
                panelWidth:800,  
                url: '<?php echo base_url(); ?>index.php/sptrdnew/load_subjenis',  
                idField:'kd_rek',                    
                textField:'kd_rek',
                mode:'remote',  
                fitColumns:true,  
	...
	function load_subjenis(){
		...
		var gol = document.frmSKPDHotel.gol.value
		$('#subjenis').combogrid({url:'<?php echo base_url(); ?>index.php/sptrdnew/load_subjenis',
                                   queryParams:({kode:gol})
        });
	}
	...
	function lihat() {
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptrdnew/lihat', postStr, responeP);			
	}
	...
	function disableData1() {
		...
		document.frmSKPDHotel.buku.disabled = false;
	...
	function enableData1() {
		...
		document.frmSKPDHotel.buku.disabled = true;
	...
	function cek(){
	  /*  if(document.frmSKPDHotel.SPTR.checked==true) {
		...
		var postCek =
            ...
            //"&jnsSPT=" + jnsSPT;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptrdnew/ricek', postCek, resPOST);
	}
	...
	function simpan2(){
		...
		var postStr =
			...
			"&buku=" + document.frmSKPDHotel.buku.value +
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptrdnew/simpan', postStr, responePOST);
	}
...
<div id="a_tabbar" style="width:1000px; height:530px; margin-left:30px;"></div>
  <div id="C1" style="background-color: #B3D9F0">
    <form name="frmSKPDHotel" id="frmSKPDHotel" action="<?php echo site_url(); ?>/sptrdnew/cetak" method="post" onSubmit="popupform(this, 'iui')" >
    <br />
   	  <table width="787" border="0">
           ...
           <!-- <td rowspan="3"><fieldset><legend>Jenis SPT</legend>
			...
			<tr>
              	<td style="padding-left:15px;">Jumlah Buku</td>
                <td colspan="2"><input type="text" name="buku" id="buku" size="10" disabled/></td>
            </tr>
			<tr>
...
<script language="javascript">
	...
	/*function tarif_pajak(){
        ...
        function get_bln_blm_bayar(){
            ...
            dhtmlxAjax.post(base_url+'index.php/sptrdnew/get_bayar_terakhir', npwpd, respBulan);
        }
	...
	function cetak1() {
		...
		window.open('<?php echo site_url(); ?>/sptrdnew/cetak?sptpd='+gabung, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	function lihat3() {
		...
		gridx.clearAll();
		gridx.loadXML("<?php echo site_url(); ?>/sptrdnew/load_perusahaan4/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	function selectedOpenData2(id) {
		...
		document.frmSKPDHotel.nama_perusahaan.value		= gridx.cells(id,3).getValue();
	...
	grid.loadXML("<?php echo site_url(); ?>/sptrdnew/data");
	grid.init();
        baru();
		...
		if (name!=null){
			if(name=='dispenda'){
				...
				if(confrm) {
					...
					statusLoading();
					dhtmlxAjax.post(base_url+'index.php/sptrdnew/delete', postStr, responeDel);	
				}
			} else {
				alert("Password anda salah");
			}
