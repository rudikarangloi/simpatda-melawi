...
<script language="javascript">
    ...
    <div style="height:100%; overflow:auto;">
        ...
        <div id="content" style="overflow: auto;">
            <h2 style="margin:30px 5px 25px 30px;">Dasar Perhitungan Reklame Insidentil</h2>
                            ...
                            <td>
                               ...
                               <!--     <option value="7">Spanduk</option>									
									...
									<option value="13">Reklame Udara</option>   -->  
									 <?php 
									foreach($jnsReklame2->result() as $rs) {
											echo "<option value=".$rs->id.">".$rs->jenis."</option>";
										} 
									?> 
                                </select></td>
...
function hapusData(){
	...
	function tambahRow(){
		...
		/* 	var tngx = tng.options[tng.selectedIndex].text				
			...
			var id = mygrid.uid();			
			var posisi = mygrid.getRowsNum();						
			mygrid.addRow(id,[btid,kws,grek,grekn,tema,kws,lks,pjg,lbr,jari,sisi,luas,unt,tng,lbrj,sdt,ktp],posisi);		//17	
			mygrid.selectRowById(id);
...
var mdp = "";
function runmdp(){
mdp = new dataProcessor("<?php echo base_url()."index.php/reklame_ins/reklame_detail"; ?>");
...
//	}
	...
	function cetak_per() {
		...
		var b = document.frmData.sptpd_2.value;
		var c = a+'/'+b;
		window.open('<?php echo site_url(); ?>/reklame_ins/cetak2?sptpd='+c, '', 'height=700,width=1000,scrollbars=yes');
	}
...
//		}
	...
	function buat_id(){
		var postCek =
			"cek_baru=" + "baru";
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/reklame_ins/buatid', postCek, resPOSTid);
	}
...
//			}
	...
	function cek(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/reklame_ins/ricek', postCek, resPOST);
	}
...
//			'&txttglmasapajak2=' + tgl2 +
            ...
            dhtmlxAjax.post("reklame_ins/simpan", encodeURI(poststr), outputResponse);
        }
	...
	gridData.setHeader("No SPTPD,NPWPD,Nama Perusahaan,Alamat Perusahaan,Lokasi Pasang,Tanggal Diterima,Cara Hitung,Masa Pajak 1,Masa Pajak 2,Tahun Pajak,Bulan 1,Bulan 2,Petugas Input,Tanggal Permohonan,Tanggal Diperiksa,Jumlah Pasang,Jumlah Bayar,Id_reklame");//17
	...
	function loadData(){
	gridData.clearAll();	
	gridData.loadXML(base_url+"index.php/reklame_ins/load_data",function() { 
        });
	...
	function selectData(id){
		...
		mygrid.clearAll();
		mygrid.loadXML("<?php echo base_url()."index.php/reklame_ins/load_detail_edit/"?>"+no);	
			...
			if(name=='dispenda'){
				...
				if(cnf) {
					var postStr =
						...
						"&sptpd_2=" + document.frmData.sptpd_2.value;
					statusLoading();
					dhtmlxAjax.post(base_url+'index.php/reklame_ins/hapus', postStr, function(loader) {
						result = loader.xmlDoc.responseText;
						alert(result);
						gridData.clearAll();
						loadData();
	...
	function cariDataWP() {	
		...
		statusLoading();
		gridWP.clearAll();	
		gridWP.loadXML(base_url+"index.php/reklame_ins/cariDataWP/"+document.frmSrc.keyword.value+"/"+document.frmSrc.parameter.value,function() {  								
		statusEnding();
...
function sumColumn(ind) {
	...
	function get_kawasan(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/reklame_ins/kawasan', postJln, respJns);			
	}
	...
	function get_ketetapan(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/reklame_ins/pokok', postKet, respKet);			
	}
