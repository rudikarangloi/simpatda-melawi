...
<h2 style="margin:30px 5px 25px 30px;">SPTPD Mineral Bukan Logam dan Batuan</h2>
	
<div id="a_tabbar" style="width:1300px; height:670px; margin-left:30px;"></div>
  ...
  <br />
			<form name="frmData" id="frmData" method="post">
				...
				<table>
					...
					<td width="25%"><input type="text" name="txtnosptpd_a" id="txtnosptpd_a" size="20" maxlength=10 style="background-color:#FFFFCC;" />&nbsp;<input type="text" name="txtnosptpd_b" id="txtnosptpd_b" size="10" style="background-color:#FFFFCC;" />
					<input type="hidden" name="buatidx" id="buatidx" />
					</td>
				...
				<tr valign="top">
					<td><input type="text" name="txt_tgl_masa_pajak3" id="txt_tgl_masa_pajak3" size="12" disabled/>&nbsp; <i>s.d</i> <input type="text" name="txt_tgl_masa_pajak4" id="txt_tgl_masa_pajak4" size="12" disabled/>&nbsp;</td>
				...
				<tr valign="top">
					<td style="padding-left:15px;" valign="top">Lokasi Proyek</td>
					<td><select name="kec" id="kec" >
                         <option value=""></option>
                        <?php
                                foreach($kec->result() as $data){
//                                    if(isset($reklame)) { $opt="selected"; } else { $opt=""; }
								echo '<option value="'.$data->kode_kecamatan.'">'.$data->nama_kecamatan.'</option>';
                                }
                            ?>
                    </select></td>
					<td style="padding-left:15px;">&nbsp;</td>
                ...
                <tr>
                    ...
                    <select name="op" onchange="get_harga();" id="op" >
                         <option value=""></option>
                        <?php
                                foreach($dop->result() as $data){
//                                    if(isset($reklame)) { $opt="selected"; } else { $opt=""; }
								echo '<option value="'.$data->id.'|'.$data->keterangan.'|'.$data->harga_pasar.'|'.$data->auto.'">'.$data->keterangan.'</option>';
                                }
                            ?>
                    </select>&nbsp;&nbsp;Volume&nbsp;&nbsp;<input type="text" id="qty" name="qty" size="7" />&nbsp;&nbsp;Harga Dasar&nbsp;&nbsp;<input type="text" id="harga" name="harga" size="7" />&nbsp;&nbsp;
                ...
                <tr>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input type="text" name="tar" id="tar" style="text-align:right" value="<?php echo $tarif; ?>" disabled/>
                        <input type="hidden" name="tarif" id="tarif" size="10" style="text-align:right" disabled/>&nbsp;%</td>
                    </tr>
                    <tr>
...
<div id="C2">
			<div style="padding:0 2px 10px 0px;">
				<div id="gridContent" height="450px" style="margin-bottom:10px;"></div>
...
var base_url = "<?php echo base_url(); ?>";
	...
	function add(){
		...
		var row6= tarif;
		var row7= setor;
		var row8= document.frmData.buatidx.value;
		var row9= '';
		gride1.addRow(id,[row1,row2,row3,row4,row5,row6,row7,row8,row9], posisi);
		gride1.showRow(id);
		var stage = 2;
		loadtotal(stage, id);
	...
	function loadtotal(stage, id) {
		...
		document.frmData.tarif.value = document.frmData.tar.value;
		var stor = document.getElementById('tmpJum').innerHTML;
		document.frmData.setoran.value = format_uang(Math.round((document.frmData.jumlah.value*document.frmData.tarif.value)/100));
	}
	...
	gride1.setHeader("Kode Rekening,Objek Pajak,Volume/Tonase (M3/Ton),Harga Pasar (Nilai Standar),Dasar Pengenaan,Tarif(%),Jumlah,id_gal,sptpd");
	gride1.setInitWidths("95,120,120,100,120,80,80,80,80");
	gride1.setColAlign("left,left,left,right,right,right,right,right,right");
	gride1.setColTypes("ro,ro,ron,ron,ron[=c2*c3],ron,ron,ron,ron");
	gride1.setNumberFormat("0,000",3,",",".");
	//gride1.setNumberFormat("0,000",4,",",".");
	gride1.setNumberFormat("0,000",6,",",".");
	gride1.setColumnHidden(7,true);
	gride1.setColumnHidden(8,true);
	gride1.setColumnHidden(5,true);
	gride1.setColumnHidden(6,true);
	...
	grid.setHeader("id, No. SPTPD, NPWPD, Nama Perusahaan, Alamat, Cara Hitung, tgl_diterima, keperluan, jenis_galian, jumlah, petugas_input, masa_pajak1, masa_pajak2, omset,tarif,id_galianc",null,["text-align:center","text-align:center","text-align:center","text-align:center"]);
	...
	grid.setInitWidths("50,150,150,170,100,100,100,100,100,100,100,100,100,100,100,100");
	grid.setColAlign("center,left,left,left,left,left,left,left,left,left,right,left,right,left,left,left");
	grid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ron,ro,ro,ro,ron,ron,ron");
	...
	grid.loadXML("<?php echo site_url(); ?>/sptpd_galianc/data_sptpd");
	grid.init();
	...
	function selectedOpenData(id) {
		...
		document.frmData.buatidx.value = grid.cells(id,15).getValue();
		//alert(sptpd);
		gride1.clearAll();
	...
	function buat_id(){
		var postCek =
			"cek_baru=" + "baru";
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptpd_galianc/buatid', postCek, resPOSTid);
	}
	...
	function enableData1(){
		...
		document.frmData.txt_tgl_masa_pajak4.disabled=false;		
		document.frmData.op.disabled=false;
		document.frmData.kec.disabled=false;
		document.frmData.qty.disabled=false;		
		document.frmData.btnsimpan.disabled=false;		
	...
	function disabledData1(){
		...
		document.frmData.kec.disabled=true;
		document.frmData.harga.disabled=false;
	...
	function simpan2(){	
		...
		+ "&buatidf=" + document.frmData.buatidx.value
		...
		+ "&kec=" + document.frmData.kec.value
	...
	function refreshData() {
		grid.clearAll();
		grid.loadXML(base_url+'index.php/sptpd_galianc/data_sptpd');
	}
	...
	function get_harga(){
		var jns_hrg = document.frmData.op.value; 
		a = jns_hrg.split('|');
		var postJln =
			"objek=" + a[3] +
			"&kec=" + document.frmData.kec.value;
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptpd_galianc/get_harga', postJln, respJns);			
	}
