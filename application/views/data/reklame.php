...
<script language="javascript">
    ...
    <div style="height:100%; overflow:auto;">
        ...
        <div id="content" style="overflow: auto;">
            <h2 style="margin:30px 5px 25px 30px;">Dasar Perhitungan Reklame Tetap</h2>

            <div id="a_tabbar" style="width:1310px; height:950px; margin-left:30px;"></div>
                        ...
                        <tr>
                        	...
                        	<td>
                                <select name="cara" id="cara" disabled="disabled">
									<option value="1" selected="selected">Official Assesment(Dihitung dan ditetapkan oleh Pejabat Pemda)</option>
                                </select>                            
							</td>
                        </tr>
                        <tr>
 ...
 <!--                       <tr>
                        ...
                        <tr>
                            <td colspan="4" style="padding-left:15px; background-color:#FFCC99;">
								...
								<input type="button" value="CETAK" onclick="cetak_per()" name="cetak2" disabled="" style="width:90px;"></td>
...
function hapusData(){
	...
	function tambahRow(){
		...
		if(document.frmData.ketetapan.value!=''){
			...
			var lok  = document.getElementById('lokasi_pasang1');
			var tng  = document.getElementById('tinggi');
			var lbrj = document.frmData.lebar.value;
			var sdt  = document.getElementById('sudut');
			...
			var lks = lok.options[lok.selectedIndex].text
			var tngx = tng.options[tng.selectedIndex].text			
			//var lbrx = lbrj.options[lbrj.selectedIndex].text			
			var sdtx = sdt.options[sdt.selectedIndex].text
			/* tngx = tngx.replace(/&gt;/g,">");
			lbrx = lbrx.replace(/&gt;/g,">");
			lbrx = lbrx.replace(/&gt;/g,">"); */
			var id = mygrid.uid();			
			var posisi = mygrid.getRowsNum();						
			mygrid.addRow(id,[btid,kws,grek,grekn,tema,kws,lks,pjg,lbr,jari,sisi,luas,unt,tngx,lbrj,sdtx,ktp],posisi);		//17	
			mygrid.selectRowById(id);
    ...
    mygrid.setHeader("btid,KawasanID, Kode Rekening, Nama Rekening, Judul Reklame, Kawasan, Lokasi, Panjang (m),Lebar (m),Jari-jari (m),Sisi,Luas (m2),Unit,Ketinggian,Lebar Jalan,Sudut Pandang,Ketetapan,no_sptpd",null,["text-align:center","text-align:center","text-align:center","text-align:center","text-align:center","text-align:center","text-align:center","text-align:center","text-align:center","text-align:center","text-align:center","text-align:center","text-align:center"]);    
    mygrid.setInitWidths("100,100,90,220,120,90,200,75,75,75,75,75,75,75,75,120,150,100");
    mygrid.setColAlign("center,center,center,left,left,center,left,center,center,center,center,center,center,center,center,center,center,left");
...
//	}
	...
	function cetak_per() {
		...
		var b = document.frmData.sptpd_2.value;
		var c = a+'/'+b;
		window.open('<?php echo site_url(); ?>/reklame/cetak2?sptpd='+c, '', 'height=700,width=1000,scrollbars=yes');
	}
...
//		
		...
		arrTgl_4 = document.frmData.tgl_permohonan.value.split("/");
		tgl_permohonan = arrTgl_4[2]+"-"+arrTgl_4[1]+"-"+arrTgl_4[0];
		
		arrTgl_5 = document.frmData.tgl_diperiksa.value.split("/");
		tgl_diperiksa = arrTgl_5[2]+"-"+arrTgl_5[1]+"-"+arrTgl_5[0];
...
//			'&txttglmasapajak2=' + tgl2 +
	...
	//gridData.setHeader("No SPTPD,NPWPD,Nama Perusahaan,Alamat Perusahaan,Lokasi Pasang,Tanggal Diterima,Cara Hitung,Masa Pajak 1,Masa Pajak 2,Tahun Pajak,Bulan 1,Bulan 2,Petugas Input,Tanggal Permohonan,Tanggal Diperiksa,Jumlah Pasang,Jumlah Bayar,Id_reklame");//17 Ori_17_Mrt
	gridData.setHeader("No SPTPD,NPWPD,Nama Perusahaan,Alamat Perusahaan,Judul Reklame,Tanggal Diterima,Cara Hitung,Masa Pajak 1,Masa Pajak 2,Tahun Pajak,Bulan 1,Bulan 2,Petugas Input,Tanggal Permohonan,Tanggal Diperiksa,Jumlah Pasang,Jumlah Bayar,Id_reklame");//17
	gridData.attachHeader("#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter");
	gridData.setInitWidths("100,100,100,400,400,100,100,100,100,100,100,100,100,100,100,100,100,100");
	gridData.setColAlign("left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,right,left");
	gridData.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ron,ro");
	...
	function selectData(id){
		...
		document.frmData.tgl_terima.value = gridData.cells(id,5).getValue(); //tgl-diterima
		document.frmData.cara.value = gridData.cells(id,6).getValue(); //cara_hitung
		document.frmData.tgl_ms_pajak1.value = gridData.cells(id,7).getValue(); //masa_pajak1
		document.frmData.tgl_ms_pajak2.value = gridData.cells(id,8).getValue(); //masa_pajak2
		document.frmData.txtthnmasapajak.value = gridData.cells(id,9).getValue(); //tahun_pajak
		document.frmData.txtblnmasapajak1.value = gridData.cells(id,10).getValue(); //bulan1
		document.frmData.txtblnmasapajak2.value = gridData.cells(id,11).getValue(); //bulan2
		document.frmData.petugas.value = gridData.cells(id,12).getValue(); //petugas
		document.frmData.tgl_permohonan.value = gridData.cells(id,13).getValue(); //tgl_permohonan
		document.frmData.tgl_diperiksa.value = gridData.cells(id,14).getValue(); //tgl_diperiksa	
		document.frmData.jml_dipasang.value = gridData.cells(id,15).getValue();
		document.frmData.sub_jumlah.value = format_number(gridData.cells(id,16).getValue()); //sub_jumlah	
		document.frmData.jml_dibayar.value = gridData.cells(id,16).getValue(); //jml_dibayar								
		//document.frmData.buatidx.value = gridData.cells(id,16).getValue(); //jml_dibayar
        //document.frmData.lokasi_pasang1.value = gridData.cells(id,5).getValue(); //lokasi_pasang1
		//document.frmData.teks_reklame.value = gridData.cells(id,4).getValue(); //judul
		//document.frmData.sifatp.value = gridData.cells(id,4).getValue(); //Sifat 
		//document.frmData.teks_reklame.value = gridData.cells(id,4).getValue(); //judul
...
function sumColumn(ind) {
	...
	function get_kawasan(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/reklame/kawasan', postJln, respJns);			
	}
	...
	function get_ketetapan(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/reklame/pokok', postKet, respKet);			
	}
