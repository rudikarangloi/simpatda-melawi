...
<h2 style="margin:30px 5px 25px 30px;">Master Hotel</h2>

<div class="oval_big2">

<div id="topnav" style="width: auto;"></div>
<div id="gridboxfrmData" style="width:500px; height:300px;"></div><br>
<input type="button" value="Tambah" onclick="addRowList()" />
<input type="button" value="Delete" onclick="deleterowsfrmData()" />
</div>

<script language="javascript">
	gridfrmData = new dhtmlXGridObject('gridboxfrmData');
	gridfrmData.setImagePath("<?php echo base_url(); ?>/assets/codebase_grid/imgs/");
	gridfrmData.setHeader("ID, Kode Hotel, Nama Hotel");
	gridfrmData.setInitWidths("90,100,150")
	...
	gridfrmData.loadXML("<?php echo site_url(); ?>/master_setting/data_hotel");
	initDP();

	var dpps = "";
	function initDP() {
		dpps = new dataProcessor("<?php echo site_url(); ?>/master_setting/data_hotel");
