...
<h2 style="margin:30px 5px 25px 30px;">Master Target Pajak1</h2>
<div class="oval_big2">

<div id="topnav" style="width: auto;"></div>
<div id="gridboxfrmData" style="width:640px; height:300px;"></div><br />
</div>
<script language="javascript">
	gridfrmData = new dhtmlXGridObject('gridboxfrmData');
	gridfrmData.setImagePath("<?php echo base_url(); ?>/assets/codebase_grid/imgs/");
	gridfrmData.setHeader("Nama Pajak, Target APBD, Target Pajak,Tahun,ID");
	gridfrmData.setInitWidths("200,150,150,90,100")
	gridfrmData.setColAlign("left,right,right,left,center")
	gridfrmData.setColTypes("ro,ed,ed,ro,ro");
	gridfrmData.setColumnHidden(4,true);
	gridfrmData.setNumberFormat("0,000",1,",",".");
	gridfrmData.setNumberFormat("0,000",2,",",".");
	gridfrmData.attachFooter("Jumlah,<div style=text-align:right>{#stat_total}</div>,<div style=text-align:right>{#stat_total}</div>,#cspan");
	gridfrmData.enableMultiselect(true);
	gridfrmData.init();
	gridfrmData.setSkin("dhx_skyblue");
	gridfrmData.loadXML("<?php echo site_url(); ?>/master_setting/data_target");
	initDP();

	var dpps = "";
	function initDP() {
		dpps = new dataProcessor("<?php echo site_url(); ?>/master_setting/data_target");
