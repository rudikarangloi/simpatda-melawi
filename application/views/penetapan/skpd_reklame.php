...
<div style="height:100%; overflow:auto;">
  <script language="javascript">
	...
	function simpan() {
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/skpd/simpan', postStr, function(loader) {
			result = loader.xmlDoc.responseText;
			arr = result.split("/");
     ...
     <table width="888" cellspacing="0">
        ...
        <tr>
          <td colspan="4" style="padding-left:15px; background-color:#FFCC99;"><input type="button" name="button" id="button" value="Baru" onclick="newEntry();" style="width:90px;" />
              ...
              <input type="hidden" value="UBAH KELURAHAN" name="ubahkel" id="ubahkel" disabled="disabled" onclick="ubahKel()" />
  ...
  <script language="javascript">
	...
	cal2.setDateFormat('%d/%m/%Y');
  	...
  	function cet(){
		...
		var marginBawah = document.frmSKPD.marginBawah.value;
		window.open('<?php echo site_url(); ?>/skpd/cetak_reklame?cetakHalaman='+cekCetakHalaman+'&marginBawah='+marginBawah+'&skpd='+gabung, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	function cariSKPD() {
		...
		statusLoading();
		gridWP.clearAll();	
		gridWP.loadXML(base_url+"index.php/skpd/cariData/"+kata_kunci+"/"+document.frmSrc5.parameter.value+"/"+document.frmSKPD.kode_sptpd.value,function() {  statusEnding(); });
	}
	...
	var dppa = "";
	function rundpPO(){
		dppa = new dataProcessor("<?php echo site_url(); ?>/skpd/dt_rekening");
	...
	grid.setHeader("No.SKPD,Tanggal,Nama Usaha,Alamat Usaha,Nama Pemilik,Alamat,NPWPD,Total,Bln Awal,Bln Akhir,Tahun,Tgl Tempo,NO.SPTPD,Nota Hitung,No Kohir,ID_ttd,ms1,ms2,Ket SKPD");
	...
	function loadData() {
		grid.clearAll();
		grid.loadXML(base_url+"index.php/skpd/mainData?kode="+document.frmSKPD.kode_sptpd.value,function() { statusEnding(); });
	}
	statusEnding();
</script>
</div>
