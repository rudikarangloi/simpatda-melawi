...
*/	}
	...
	function simpan() {
		...
		if(document.frmNHP.tahun.value==""){
			var x = document.frmNHP.sptpd.value;		
			x1 = x.split("/");	
			thnx = x1[2];	
		}else{
			thnx = document.frmNHP.tahun.value;
		}
...
<div id="a_tabbar" style="width:1000px; height:560px; margin-left:30px;"></div>
...
<script language="javascript">
	...
	function lihatNHP() {
		...
		grids.clearAll();
		grids.loadXML("<?php echo site_url(); ?>/nhp/load_cnhp/"+op+"/"+nilai+"/"+,function() {
			statusEnding();
		});
	}
	...
	grids.setHeader("id, npwpd, no_sptpd, nama_perusahaan, alamat_perusahaan, nama_pemilik, alamat_pemilik, cara_hitung, tgl_terima, masa_pajak1, masa_pajak2, jml_bayar");
	...
	grids.setColAlign("center,left,left,left,left,left,left,left,left,left,left,right");
	grids.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ron");
	grids.setColSorting("str,str,str,str,str,str,str,str,str,str,str");
	//grids.enablePaging(true,5,10,"pagingArea",true);
	grids.setPagingSkin("bricks");
	grids.setColumnHidden(0,true);
	grids.setNumberFormat("0,000",11,",",".");
