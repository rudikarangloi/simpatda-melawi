...
<script language="javascript">
	...
	function lihat() {
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/izin_dl/lihat_pemilik', postStr, responeP);			
	}
	...
	function lihat2() {
		...
		grid.clearAll();
		grid.loadXML("<?php echo site_url(); ?>/izin_dl/load_perusahaan/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	function simpan() {
		...
		var pemStr = 
			"id_pemilik=" + document.frmPerusahaan.id_pemilik.value;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/izin_dl/cek_pemilik', pemStr, pemPOST);
	}
...
//		}
		...
		dhtmlxAjax.post(base_url+'index.php/izin_dl/simpan_perusahaan', postStr, responePOST);
	}
	...
	function ckelurahan() {
		var postStr =
			"kelurahan=" + document.frmPerusahaan.kelurahan.value;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/izin_dl/ckel', postStr, wenePOST);
	}
	...
	function ckecamatan() {
		...
		statusLoading();
		//alert(postStr);
		dhtmlxAjax.post(base_url+'index.php/izin_dl/ckec', postStr, wenePOST);
	}
    ...
    function ckecamatan_pemilik() {
		...
		statusLoading();				
		dhtmlxAjax.post(base_url+'index.php/izin_dl/ckec_2', postStr2, pemilikPOST);
	}
...
<h2 style="margin:30px 5px 25px 30px;">Pendaftaran Usaha Data Lama</h2>

<div id="a_tabbar" style="width:1280px; height:1160px; margin-left:30px;"></div>
  <div id="C1" style="width:1050px; background-color: #B3D9F0; padding-bottom:10px;"><br />
    <form name="frmPerusahaan" id="frmPerusahaan" action="<?php echo site_url(); ?>/izin_dl/cetak_perusahaan" method="post" onSubmit="popupform(this, 'Perusahaan')">
    	<table>
            <tr>
               <!-- <td width="250" style="padding-left:15px;"><strong>NPWPD Usaha</strong></td>-->
                <td><input type="hidden" name="id_perusahaan" id="id_perusahaan" size="45" style="background-color:#FFFFCC;" disabled/>
           ...
           <table>
            ...
            <tr>
                ...
                <td><input type="text" name="kabupaten_p" id="kabupaten_p" value="KOTA JAMBI" size="45" style="background-color:#FFFFCC;" disabled="disabled" /></td>
            </tr>
            </table>
            </fieldset>
            </td>
			...
			<tr> <!--BOWO-->
                ...
                foreach($reke->result() as $rs1) {
                        echo "<option value=".$rs1->auto.">".$rs1->nm_rekening."</option>";
                    }
                ?>
                </select></td>
            </tr>
			<tr>
                ...
                <td><input type="text" name="sptpd" id="sptpd" disabled="disabled" size="30"/>
...
<br />
    ...
    <div style="padding:0 0px 10px 0px;">
   		<div id="gridContent" height="510px" width="1270px" style="margin-bottom:10px;"></div>
...
<script language="javascript">
	...
	function lihatPem() {
		...
		grids.clearAll();
		grids.loadXML("<?php echo site_url(); ?>/izin_dl/load_pemilik/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	//grids.loadXML("<?//php echo site_url(); ?>/izin_dl/data_pemilik");
	grids.init();
	...
	function lihat21() {
		...
		gridData.clearAll();
		gridData.loadXML("<?php echo site_url(); ?>/izin_dl/load_registrasi/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
...
//		}
	...
	function crek_pajak() {
		...
		statusLoading();
		//alert(postStr);
		dhtmlxAjax.post(base_url+'index.php/izin_dl/crek_pajak', postStr, wenePOST_rek);
	}
	...
	grid.setHeader("id, NPWPD Usaha Baru, NPWPD Usaha Lama, nama_Usaha, alamat_Usaha, rt, rw, email, kelurahan, nama_kelurahan, kecamatan, nama_kecamatan, kabupaten, telp, kodepos, npwpd_pemilik, nama_pemilik, alamat_pemilik, jalan_pemilik, rt_pemilik, rw_pemilik, email_pemilik, lokasi_pemilik, kelurahan_pemilik, kecamatan_pemilik, kabupaten_pemilik, telp_pemilik, kodepos_pemilik, jenis_usaha, status_usaha, kategori, jml_karyawan, ukuran_tempat, surat_izin, no_surat, tgl_surat, jenis_pajak, gol_pajak, tgl_daftar,idx,idauto");//36
	grid.setInitWidths("50,150,150,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,150,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100");//19
	grid.setColAlign("center,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left");
	grid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");
	grid.setColSorting("str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str.str");
	...
	grid.loadXML("<?php echo site_url(); ?>/izin_dl/data_perusahaan");
	grid.init();
	...
	function cetak1() {
		window.open('<?php echo site_url(); ?>/izin_dl/cetak_peru?npwpd='+document.frmPerusahaan.id_perusahaan.value, '', 'height=700,width=1000,scrollbars=yes');
	}
	
	function cetak2() {
		window.open('<?php echo site_url(); ?>/izin_dl/cetak_peru2?npwpd='+document.frmPerusahaan.id_perusahaan.value, '', 'height=700,width=1000,scrollbars=yes');
	}
			...
			if(enter=='dispenda'){
				...
				if(confrm) {
					var postStr =
						"id_perusahaan=" + document.frmPerusahaan.id_perusahaan.value;
						statusLoading();
						dhtmlxAjax.post('<?php echo site_url(); ?>/izin_dl/delete_perusahaan', postStr, responeDel);			
				}
			} else {
