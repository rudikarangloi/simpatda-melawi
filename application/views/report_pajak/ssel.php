...
<div align="center">
	...
	<h2>Laporan SPTPD Sudah Bayar (Self Assesment)</h2>
	<form name="tetap" id="tetap">
    	<table>
            ...
            <tr>
            	<td colspan="2" align="center"><input type="button" name="pdf" id="pdf" onclick="tpdf();" value="Cetak to PDF" style="padding-left:20px; padding-right:20px" disabled/>&nbsp;<input type="button" name="xls" id="xls" onclick="txls();" value="Cetak to Excel" style="padding-left:20px; padding-right:20px" disabled/></td>
            </tr>
        </table>
    </form>
    
<div id="gridbox" width="1100px" height="490px" style="margin-bottom:10px;"></div>
</div>
...
function tpdf(){
	...
	window.open('<?php echo site_url(); ?>/report_pajak/cetak_ssel?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function txls(){
	...
	full = awal+'|'+akhir+'|'+document.tetap.jenis_pajak.value;
	window.open('<?php echo site_url(); ?>/report_pajak/excel_ssel?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function tampil(){
	...
	mygrid.clearAll();
	mygrid.loadXML("<?php echo site_url(); ?>/report_pajak/data_sself/"+awal+"/"+akhir+"/"+document.tetap.jenis_pajak.value,function() {
...
mygrid.setHeader("NO. ,TANGGAL & NO. BUKTI,#cspan,#cspan,#cspan,NAMA WAJIB PAJAK,ALAMAT,NPWPD,KETETAPAN,DENDA,PENYETORAN",
	null,["text-align:center","text-align:center"]);
mygrid.attachHeader(["#rspan", "  TGL SPTPD  ", "NO. SPTPD", "TGL SETOR", "NO. SSPD", "#rspan", "#rspan", "#rspan","#rspan", "#rspan","#rspan"]);
...
mygrid.setInitWidths("40,70,100,70,120,200,100,100,100,100,100");
mygrid.setColAlign("center,center,left,center,left,left,left,left,right,right,right");
mygrid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ron,ron,ron");
mygrid.setNumberFormat("0,000",8,",",".");
mygrid.setNumberFormat("0,000",9,",",".");
mygrid.setNumberFormat("0,000",10,",",".");
mygrid.setColSorting("str,str,int,int,int,int,int,int,int,int,int");
mygrid.setSkin("dhx_skyblue");
mygrid.attachFooter("Jumlah,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,<div style=text-align:right id=tmpDp>{#stat_total}</div>,<div style=text-align:right id=tmpTar>{#stat_total}</div>,<div style=text-align:right id=tmpNaik>{#stat_total}</div>");
mygrid.init();
</script>