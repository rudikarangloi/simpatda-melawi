...
<div align="center">
	...
	<h2>Laporan Realisasi Penerimaan Pajak</h2>
	<form name="tetap" id="tetap">
    	<table>
            ...
            <tr align="center">
                ...
                <input type="button" name="tpdf" id="tpdf" onclick="pdf();" value="Cetak to PDF" style="padding-left:20px; padding-right:20px" disabled/>&nbsp;<!--<input type="button" name="xls" id="xls" onclick="txls();" value="Cetak to Excel" style="padding-left:20px; padding-right:20px" disabled />-->
                </td>
            </tr>
        </table>
    </form>
...
function pdf() {
	...
	window.open('<?php echo site_url(); ?>/report_pajak/cetak_realisasi?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function tampil(){
	...
	mygrid.clearAll();
	mygrid.loadXML("<?php echo site_url(); ?>/report_pajak/data_realisasi/"+document.tetap.jenis_pajak.value+"/"+document.tetap.tahun.value,function() {
...
mygrid.setHeader("No. ,T.M.T ,Nama Wajib Pajak, Alamat, Tahun Lalu,Januari,Februari,Maret,April,Mei,Juni,Juli,Agustus,September,Oktober,November,Desember",//17
...
mygrid.attachHeader(["#rspan","#rspan", "#rspan", "#rspan", "#rspan", "Realisasi", "Realisasi", "Realisasi", "Realisasi", "Realisasi", "Realisasi", "Realisasi", "Realisasi", "Realisasi", "Realisasi", "Realisasi", "Realisasi"]);
mygrid.setInitWidths("40,100,200,300,100,100,100,100,100,100,100,100,100,100,100,100,100");
mygrid.setColAlign("center,center,left,left,right,right,right,right,right,right,right,right,right,right,right,right,right");
mygrid.setColTypes("ro,ro,ro,ro,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron");
...
mygrid.setColSorting("str,str,str,str,int,int,int,int,int,int,int,int,int,int,int,int,int");
mygrid.setSkin("dhx_skyblue");
mygrid.attachFooter("Total,#cspan,#cspan,#cspan,<div style=text-align:right id=s0>{#stat_total}</div>,<div style=text-align:right id=s1>{#stat_total}</div>,<div style=text-align:right id=s2>{#stat_total}</div>,<div style=text-align:right id=s3>{#stat_total}</div>,<div style=text-align:right id=s4>{#stat_total}</div>,<div style=text-align:right id=s5>{#stat_total}</div>,<div style=text-align:right id=s6>{#stat_total}</div>,<div style=text-align:right id=s7>{#stat_total}</div>,<div style=text-align:right id=s8>{#stat_total}</div>,<div style=text-align:right id=s9>{#stat_total}</div>,<div style=text-align:right id=s10>{#stat_total}</div>,<div style=text-align:right id=s11>{#stat_total}</div>,<div style=text-align:right id=s12>{#stat_total}</div>");
mygrid.init();
</script>