...
if($type=='excel'):
	...
	header('Content-Disposition: attachment; filename=lap_penerimaan_reklame.xls');	
endif;
?>
...
<table border=1>
	<tr>
		...
		<td align="center"><strong>UKURAN REK</strong></td>
...
a.masa_pajak_1,a.masa_pajak_2,a.no_sspd,
c.dp,c.denda,c.jumlah,a.setoran,a.status,DATE_FORMAT(d.tgl_bayar,'%d/%m/%Y') AS tgl_bayar,
c.kode_rekening,c.nama_rekening,DATE_FORMAT(a.tanggal,'%d/%m/%Y') AS tanggal,f.lokasi,
f.panjang,f.lebar
...
LEFT JOIN sptpd_reklame_detail f ON e.sptpd=f.no_sptpd
...
");
		...
		foreach($rc->result() as $rs){
			...
			$masa1			= explode("-",$rs->masa_pajak_1);
			$dd1				= $masa1[0];
			$dd2				= $this->msistem->c_bln($masa1[1]);
			$dd3				= $masa1[2];
			$masa2			= explode("-",$rs->masa_pajak_2);
			$de1				= $masa2[0];
			$de2				= $this->msistem->c_bln($masa2[1]);
			$de3				= $masa2[2];
			...
			echo
				'<tr>
					...
					<td align="left">REK.'.$rs->nama_rekening.'</td>
					<td align="left">'.$rs->lokasi.'</td>
					<td align="left">'.$rs->npwpd.'&nbsp;</td>
					<td align="center">'.$rs->panjang.'x'.$rs->lebar.'&nbsp;</td>
					<td align="left">'.$rs->no_sspd.'&nbsp;</td>
					<td align="center">'.$dd2." ".$dd1.' - '.$de2." ".$de1.'</td>
