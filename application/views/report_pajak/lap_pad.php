...
<div align="center">
	...
	<h2>Laporan Realisasi Pajak Dan Retribusi</h2>
	<form name="tetap" id="tetap">
    	<table>
        	<tr>
            	<td style="padding-left:15px;">Bulan</td>
                <td><input type="text" name="awal" id="awal" size="15" />&nbsp;<input type="text" name="akhir" id="akhir" size="15" disabled="true" value="Tidak Perlu di Isi" />
            ...
            <!--<tr>
                ...
                <input type="hidden" value="Display" onclick="tampil()" />&nbsp;
                <input type="button" name="tpdf" id="tpdf" onclick="pdf();" value="Cetak to PDF" style="padding-left:20px; padding-right:20px" />
			...
			<tr>
				...
				<input type="button" name="xls" id="xls" onclick="cetak(2);" value="Cetak ke Excel" style="padding-left:20px; padding-right:20px" />
				</td>
            </tr>
        </table>
    </form>
...
/* function cetak_rekap() {
	...
	full = marginBawah;
	window.open('<?php echo site_url(); ?>/report_pajak2/cetak_piutang_rekap?full='+full, '', 'height=700,width=1000,scrollbars=yes');
} */
function cetak(pilih) {
	...
	akhir = p[2]+'-'+p[1]+'-'+p[0];
 	...
 	var marginBawah = document.tetap.marginBawah.value;
	full = awal+'|'+akhir+'|'+marginBawah+'|'+pilih;
	window.open('<?php echo site_url(); ?>/report_pajak2/cetak_lap_pad?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
/* function cetak_rekap() {
	...
	akhir = p[2]+'-'+p[1]+'-'+p[0];
 	...
 	var marginBawah = document.tetap.marginBawah.value;
	full = awal+'|'+akhir+'|'+marginBawah;
	window.open('<?php echo site_url(); ?>/report_pajak2/cetak_jurnal?full='+full, '', 'height=700,width=1000,scrollbars=yes');
} */


function cetak_penyisihan() {
 	var marginBawah = document.tetap.marginBawah.value;
	full = marginBawah;
	window.open('<?php echo site_url(); ?>/report_pajak2/cetak_piutang_penyisihan?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function pdf() {
	...
	window.open('<?php echo site_url(); ?>/report_pajak2/cetak_piutang?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function txls(){
	...
	window.open('<?php echo site_url(); ?>/report_pajak2/excel_piutang?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
mygrid.setHeader("No. ,No.SKPD, Tanggal SKP,Tanggal SKP , Uraian, Kode Rekening, Nama Rekening, Pokok, Denda, Penerimaan",//9
...
mygrid.setInitWidths("40,120,100,100,250,100,220,110,110,110");
mygrid.setColAlign("center,center,center,center,left,center,left,right,right,right");
mygrid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ron,ron,ron");
mygrid.setNumberFormat("0,000",6,",",".");
mygrid.setNumberFormat("0,000",7,",",".");
mygrid.setNumberFormat("0,000",8,",",".");
mygrid.setNumberFormat("0,000",9,",",".");
mygrid.setColSorting("str,str,str,str,str,str,str,int,int,int");
mygrid.setSkin("dhx_skyblue");
mygrid.attachFooter("Jumlah Penerimaan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,<div style=text-align:right id=tmpDp>{#stat_total}</div>,<div style=text-align:right id=tmpTar>{#stat_total}</div>,<div style=text-align:right id=tmpDp2>{#stat_total}</div>");
