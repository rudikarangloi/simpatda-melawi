...
function pdf(){
	...
	window.open('<?php echo site_url(); ?>/report_pajak/cetak_kartu?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function tampil(){
	...
	statusLoading();
	mygrid.clearAll();
	mygrid.loadXML("<?php echo site_url(); ?>/report_pajak/data_kartu/"+document.kartu.npwpd.value+"/"+aw+"/"+ak,function() {
		cariData();
		statusEnding();
	});
}
...
mygrid.setHeader("NO., NO. SSPD, TGL PEMBAYARAN, MASA PAJAK, TAHUN, JUMLAH PAJAK, DENDA, REALISASI, KET",
...
mygrid.setInitWidths("40,150,170,150,90,150,100,150,200");
mygrid.setColAlign("center,center,center,center,center,right,right,right,left");
mygrid.setColTypes("ro,ro,ro,ro,ro,ron,ron,ron,ro");
mygrid.setNumberFormat("0,000",5,",",".");
mygrid.setNumberFormat("0,000",6,",",".");
mygrid.setNumberFormat("0,000",7,",",".");
mygrid.setColSorting("str,str,str,str,int,int,int,int,str");
mygrid.setSkin("dhx_skyblue");
mygrid.attachFooter("Jumlah,#cspan,#cspan,#cspan,#cspan,<div style=text-align:right id=tmpDp>{#stat_total}</div>,<div style=text-align:right id=tmpTar>{#stat_total}</div>,<div style=text-align:right id=tmpNaik>{#stat_total}</div>");
mygrid.init();
</script>