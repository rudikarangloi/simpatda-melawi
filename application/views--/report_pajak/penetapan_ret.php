...
<div align="center">
	...
	<h2>Laporan Register Penetapan Retribusi</h2>
...
<div id="gridbox" width="980" height="550px" style="margin-bottom:10px;"></div>
</div>
...
function cetak(pilih) {
	...
	window.open('<?php echo site_url(); ?>/report_pajak2/cetak_penetapan_ret?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function tampil(){
	...
	statusLoading();	
	mygrid.clearAll();
	mygrid.loadXML("<?php echo site_url(); ?>/report_pajak2/data_penetapan_ret/"+awal+"/"+akhir+"/"+pajak,function() {
		/* awal2 = s[2]+'-01-01';
		if(akhir2<awal2){
...
mygrid.setHeader("No. ,SKPD Pengelola, Jenis Karcis,Petugas Penagih, Buku, Nominal, Total, No Seri Karcis",//6, dihitung dari 0
...
mygrid.setInitWidths("40,250,250,180,110,110,110,110");
mygrid.setColAlign("center,center,center,center,center,right,right,center");
mygrid.setColTypes("ro,ro,ro,ro,ro,ro,ron,ro");
mygrid.setNumberFormat("0,000",6,",",".");
mygrid.setColSorting("str,str,str,str,str,str,str,int");
mygrid.setSkin("dhx_skyblue");
mygrid.attachFooter("Jumlah Penetapan,#cspan,#cspan,#cspan,<div style=text-align:right id=tmpDp>{#stat_total}</div>,<div style=text-align:right id=tmpTar>{#stat_total}</div>,<div style=text-align:right id=tmpDp2>{#stat_total}</div>");
