...
<div align="center">
	...
	<h2>Laporan SKP Belum Bayar (Official Assesment)</h2>
...
<div id="gridbox" width="1150px" height="490px" style="margin-bottom:10px;"></div>
</div>
...
function tpdf(){
	...
	window.open('<?php echo site_url(); ?>/report_pajak/cetak_belum?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function txls(){
	...
	full = awal+'|'+akhir+'|'+document.tetap.jenis_pajak.value;
	window.open('<?php echo site_url(); ?>/report_pajak/excel_belum?full='+full, '', 'height=700,width=1000,scrollbars=yes');
}
...
function tampil(){
	...
	mygrid.clearAll();
	mygrid.loadXML("<?php echo site_url(); ?>/report_pajak/data_belum/"+awal+"/"+akhir+"/"+document.tetap.jenis_pajak.value,function() {
...
mygrid.setHeader("NO.,TANGGAL,NO. SK,NAMA WAJIB PAJAK,ALAMAT,NPWPD,KETETAPAN,TANGGAL TEMPO,MASA PAJAK, KETERANGAN",
...
mygrid.setInitWidths("40,70,100,180,200,100,100,100,150,100");
mygrid.setColAlign("center,center,center,left,left,center,right,center,center,center");
mygrid.setColTypes("ro,ro,ro,ro,ro,ro,ron,ro,ro,ro");
mygrid.setNumberFormat("0,000",6,",",".");
mygrid.setColSorting("str,str,str,str,str,str,int,str,str,str");
mygrid.setSkin("dhx_skyblue");
mygrid.attachFooter("Jumlah,#cspan,#cspan,#cspan,#cspan,#cspan,<div style=text-align:right id=tmpDp>{#stat_total}</div>,<div style=text-align:right id=tmpTar>{#stat_total}</div>,<div style=text-align:right id=tmpNaik>{#stat_total}</div>,<div style=text-align:right id=tmpNaik>{#stat_total}</div>");
mygrid.init();
</script>