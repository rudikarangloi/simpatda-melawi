...
<div align="center">
	...
	<h2>Laporan Register Penetapan Perwajib Pajak</h2>
	<form name="tetap" id="tetap">
    	<table>
           ...
           <!-- <tr>
                ...
                <td><select name="jenis_pajak" id="jenis_pajak" onchange="get_wp();">
            ...
            <option value="2">Nama Usaha</option>		
				...
				<input type="text" name="values" id="values" size="35" style="text-transform:uppercase; background-color: #FFCC99;" /> 
                <input type="button" value="Display" onclick="tampil()" />&nbsp;
                <input type="hidden" name="tpdf" id="tpdf" onclick="pdf();" value="Cetak to PDF" style="padding-left:20px; padding-right:20px" />
                </td>
            </tr>
        </table>
    </form>
...
function get_wp(){
		//alert(document.tetap.jenis_pajak.value);
		var postJln =
			"wp=" + document.tetap.jenis_pajak.value;
		
		//statusLoading();
		dhtmlxAjax.post(base_url+'index.php/report_pajak2/wp', postJln, respJns);
...
function respJns(loader) {
		if (loader.xmlDoc.readyState == 4) {
			if (loader.xmlDoc.status == 200) {
				...
				//statusEnding();
				if(result!=0) {
					document.tetap.wajib_pajak.value = result;
...
function tampil(){
	...
	op = document.tetap.cek.value;
		if(op==1||op==2){
			if(document.tetap.values.value=="") {
			alert("Value Pencarian Tidak Boleh Kosong");
			document.tetap.values.focus();
			return;
			}
		}
	nilai = document.tetap.values.value; 
	t = document.tetap.awal.value;
	s = t.split("/");
	awal = s[2]+'-'+s[1]+'-'+s[0];
	...
	statusLoading();	
	mygrid.clearAll();
	mygrid.loadXML("<?php echo site_url(); ?>/report_pajak2/data_penetapan2/"+awal+"/"+akhir+"/"+nilai+"/"+op,function() {
		/* awal2 = s[2]+'-01-01';
		 if(akhir2<awal2){
			document.getElementById('tmpSDp').innerHTML=0;
			document.getElementById('tmpSTar').innerHTML=0;
		} else {
			lastDay(awal2,akhir2);
		}  */
...
mygrid.setHeader("No. ,No.SKPD, Tanggal SKP,Masa Pajak , Uraian, Kode Rekening, Nama Rekening, Pokok, Denda, Penerimaan",//9
...
mygrid.setInitWidths("40,120,100,130,250,100,220,110,110,110");
...
mygrid.attachFooter("Jumlah Penetapan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,<div style=text-align:right id=tmpDp>{#stat_total}</div>,<div style=text-align:right id=tmpTar>{#stat_total}</div>,<div style=text-align:right id=tmpDp2>{#stat_total}</div>");
