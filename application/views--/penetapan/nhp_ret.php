...
<script language="javascript">	
	function lihat() {
		if(document.frmNHP.sptpd.value=="") {
			alert("SPTRD Tidak Boleh Kosong");
			document.frmNHP.sptpd.focus();
			return;
		}
		//alert(document.frmNHP.sptpd.value);
		var postStr =
		"sptrd=" + document.frmNHP.sptpd.value +
		"&kode=" + document.frmNHP.kode.value;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/nhp_ret/lihat', postStr, responeP);			
	}
		...
		if (loader.xmlDoc.readyState == 4) {
			if (loader.xmlDoc.status == 200) {
				...
				if(result!=0) {
					...
					alert('Maaf No SPTRD tersebut sudah terdaftar');	
				}
				loadtotal();
			} else {
...
*/	}
	...
	function simpan() {
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/nhp_ret/simpan', postStr, responePOST);
	}
	...
	function tampil() {
		...
		wNHP2.center();
		wNHP2.attachObject('objBrg');
		document.frmSrc.sav.disabled = true;
	}
...
<h2 style="margin:30px 5px 25px 30px;">Nota Hitung <?php echo $title; ?></h2>

<div id="a_tabbar" style="width:1100px; height:560px; margin-left:30px;"></div>
  <div id="C1" style="background-color: #B3D9F0">
    ...
    <input type="hidden" name="nota" id="nota" size="35"  />
   	  <table width="850" border="0">
      		<tr>
                ...
                <td width="150">Tanggal &nbsp;<input type="text" name="tgl" id="tgl" size="10" value="<?php echo $tgl; ?>" disabled/></td>
        	</tr>
            <tr>
                ...
                <option value="4">SKRKBT</option>
                <option value="5">SKRKB</option>
                </select></td>
    		</tr>
            <tr>
            ...
            <tr>
                ...
                <td colspan="3"><input type="text" name="alamat_perusahaan" id="alamat_perusahaan" size="45" disabled/></td>
            </tr>
            <tr>
    ...
    <div style="padding:10px 15px 0 15px;">
   		<div id="gridCo" width="1040px" height="150px" style="margin-bottom:10px;"></div>
    </div>
</td>
</tr>
	<tr>
	...
	<tr>
		<td style="padding-left:15px;">Cari data berdasarkan 
      	<select name="fil_nhp" id="fil_nhp" >
        	<option value="1">No. SPTRD</option>
...
<script language="javascript">
	function cetak1(){
		...
		window.open('<?php echo site_url(); ?>/nhp_ret/cetak?nota='+gabung, '', 'height=700,width=1000,scrollbars=yes');
	}
...
//		}
		...
		grids.clearAll();
		grids.loadXML("<?php echo site_url(); ?>/nhp_ret/load_cnhp/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	grids.setHeader("id, npwpd, no_sptrd, nama_perusahaan, alamat_perusahaan, nama_pemilik, alamat_pemilik, cara_hitung, tgl_terima, masa_pajak1, masa_pajak2, jml_bayar");
	...
	function selectedOpenData2(id) {
        ...
        //var no   = grids.cells(id,2).getValue().split('/').join('.');
        //gride.clearAll()
		//gride.loadXML("<?php echo base_url()."index.php/nhp_ret/load_detail/"?>"+no);
...
//		}
	...
	function child_retribusi(){
		bongkar = document.frmNHP.sptpd.value.split('/');
		pasang = bongkar[0]+'-'+bongkar[1]+'-'+bongkar[2];
		//alert('pasang')
		gride.clearAll()
		gride.loadXML("<?php echo site_url(); ?>/nhp_ret/child_retribusi/"+pasang,function() {
			loadtotal();
			statusEnding();
		});
	}
	...
	gride.setHeader("Kode Rekening, Nama Rekening, Izin usaha, Volume(m2), DP, Tarif, Denda, Bunga, Jumlah, SPTRD, No Nota");
	gride.setInitWidths("100,130,320,70,100,70,70,80,100,100,100");
	gride.setColAlign("left,left,left,right,right,right,right,right,right,right,left");
	gride.setColTypes("ro,ro,ro,ron,ron,ron,ron,ron,ron,ron,ron");
	gride.setColSorting("str,str,str,int,int,int,int,int,int,str,str");
	...
	var dpps = "";
	function rundpPO(){
		dpps = new dataProcessor("<?php echo site_url(); ?>/nhp_ret/dt_rekening");
	...
	function cek(){
		gride.clearAll();
		gride.loadXML("<?php echo site_url(); ?>/nhp_ret/data_rekening?nota="+document.frmNHP.nota.value);
	}
	
	grid = new dhtmlXGridObject('gridContent');
	grid.setImagePath("<?php echo base_url(); ?>/assets/codebase_grid/imgs/");
	grid.setHeader("id, no_nota, sptpd, npwpd, cara_hitung, nama, alamat, nama_perusahaan, alamat_perusahaan, masa_pajak1, masa_pajak2, tahun, tgl, sk, kep_proyek, jumlah, kode",
	...
	//grid.attachHeader("#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter");
	grid.setInitWidths("50,170,150,150,100,100,100,100,100,100,100,100,100,100,100,100,100");
	grid.setColAlign("center,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left");
	grid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ron,ro");
	...
	grid.loadXML("<?php echo site_url(); ?>/nhp_ret/data?kode="+document.frmNHP.kode.value);
	grid.init();
			...
			if(name=='passwordnyadpka'){
				...
				if(confrm) {
					var postStr =
						...
						"&sk=" + document.frmNHP.sk.value;
					statusLoading();
					dhtmlxAjax.post(base_url+'index.php/nhp_ret/delete', postStr, responeDel);	
				}
			} else {
				alert("Password anda salah");
			}
