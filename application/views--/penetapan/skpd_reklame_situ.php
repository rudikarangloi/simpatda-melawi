  ...
  <script language="javascript">
	...
	function simpan() {
		...
		if(document.frmSKPD.ttd_skpd.value=="") {
			...
			alert("Tanda Tanggan Tidak Boleh Kosong");
			return;
		}
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/skpd_situ/simpan', postStr, function(loader) {
			result = loader.xmlDoc.responseText;
			arr = result.split("/");
	...
	function ubahKel(){
		var npwpd = "";			
		npwpd = document.frmSKPD.npwpd.value;
		var win = window.open('../../../../defri/ubah_kelurahan/ubah_kelurahan.php?npwpd='+npwpd,'_blank');
		win.focus();
	}
			...
			if(name=='dispenda'){
				...
				if(cnf) {
					var postStr =
						...
						"&skpd_2=" + document.frmSKPD.skpd_2.value;
					statusLoading();
					dhtmlxAjax.post(base_url+'index.php/skpd_situ/hapus', postStr, responeDel);	
				}
			} else {
	...
	function bersih() {
		...
		document.frmSKPD.ket_ms1.value = "";
		document.frmSKPD.ket_ms2.value = "";
...
body{
  ...
  <h2 style="margin:30px 5px 25px 30px;">SKP <?php echo $nama_sptpd; ?></h2>
  <div id="a_tabbar" style="width:1000px; height:590px; margin-left:30px;"></div>
  <div id="C1" style="background-color: #B3D9F0">
    <form name="frmSKPD" id="frmSKPD" action="<?php echo site_url(); ?>/skpd_situ/cetak" method="post" onsubmit="popupform(this, 'skpd')" ><input name="skpd" type="hidden" id="skpd" size="8" /><br />
     <table width="888" cellspacing="0">
        ...
        <tr>
		  ...
		  <td rowspan="4"><textarea name="ket_skpd" id="ket_skpd" cols="25" rows="4"></textarea></td>
        </tr>
        <tr>
		...
		<tr>
          ...
          <td><input name="ket_ms1" type="text" id="ket_ms1" size="15" disabled="disabled" /> s/d <input name="ket_ms2" type="text" id="ket_ms2" size="15" disabled="disabled" /></td>
...
<input name="cetakHalaman" type="checkbox" id="cetakHalaman" />
Cetak Halaman </td>
        </tr>
      </table>
    </form>
  ...
  <script language="javascript">
	...
	cal2.setDateFormat('%d/%m/%Y');
  	...
  	function cet(){
		...
		var cekCetakHalaman = document.frmSKPD.cetakHalaman.checked;
		window.open('<?php echo site_url(); ?>/skpd_situ/cetak_reklame?cetakHalaman='+cekCetakHalaman+'&skpd='+gabung, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	gridWP.setHeader("No.Nota,SPTPD,NPWPD,Nama Perusahaan,Alamat Perusahaan,Pemilik,Alamat Pemilik,masa_pajak_1,masa_pajak_2,tahun,jml,ms1,ms2,tgl_tempo,tgl");
	gridWP.setInitWidths("120,120,120,200,200,180,180,80,80,80,80,80,80,80,80");
	...
	function cariSKPD() {
		...
		statusLoading();
		gridWP.clearAll();	
		gridWP.loadXML(base_url+"index.php/skpd_situ/cariData/"+kata_kunci+"/"+document.frmSrc5.parameter.value+"/"+document.frmSKPD.kode_sptpd.value,function() {  statusEnding(); });
	}
	...
	var dppa = "";
	function rundpPO(){
		dppa = new dataProcessor("<?php echo site_url(); ?>/skpd_situ/dt_rekening");
	...
	function loadTgl(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/skpd_situ/cektempo', poStr, respeP);			
	}
	...
	function loadItemPajak() {
		gridData.clearAll();
		statusLoading();
		gridData.loadXML(base_url+"index.php/skpd_situ/dataItemPajak?skpd="+document.frmSKPD.skpd.value,function() { statusEnding(); });
	}
	...
	function loadItem(){
		...
		pasang = b[0]+'-'+b[1]+'-'+b[2]; 
		gridData.clearAll();
		gridData.loadXML(base_url+"index.php/skpd_situ/dataItemPajak1/"+pasang,function() { statusEnding(); });			
	}
	
	// Wajib Pajak
	grid = new dhtmlXGridObject('tmpGrid');
	grid.setImagePath("<?php echo base_url(); ?>/assets/codebase_grid/imgs/");
	grid.setHeader("No.SKPD,Tanggal,Nama Usaha,Alamat Usaha,Nama Pemilik,Alamat,NPWPD,Total,Bln Awal,Bln Akhir,Tahun,Tgl Tempo,NO.SPTPD,Nota Hitung,No Kohir,ID_ttd,ms1,ms2");
	...
	grid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ron,ron,ro,ro,ro,ro,ro,ro,ro,ro,ro");
	...
	function loadData() {
		grid.clearAll();
		grid.loadXML(base_url+"index.php/skpd_situ/mainData?kode="+document.frmSKPD.kode_sptpd.value,function() { statusEnding(); });
	}
	statusEnding();
</script>
</div>
