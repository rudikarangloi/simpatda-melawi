...
<script language="javascript">
    ...
    <div style="height:100%; overflow:auto;">
        <div id="content" style="overflow: auto;">
            <h2 style="margin:30px 5px 25px 30px;">SSPD - Surat Setoran Pajak/Retribusi Daerah </h2>
			<div id="a_tabbar" style="width:900px; height:720px; margin-left:30px"></div>
            <div id="C1" style="background-color: #B3D9F0">
                <form name="frmSSPD" id="frmSSPD" action="<?php echo site_url(); ?>/sspd_situ/cetak" method="post" onSubmit="popupform(this, 'sspd')" >
                    <br />
                  ...
                  <td width="319"><input type="text" name="nama_perusahaan" id="nama_perusahaan" size="45" disabled/></td>
                        ...
                        <tr>
                            ...
                            <td><select id="ttd_sspd" name="ttd_sspd">
								...
								<option value="1">Dina Hermina. SE</option>
								<option value="2">Astri Liliani, SE.AK</option>								
								<option value="3">Desyanto</option>								
							</select></td>
                        </tr>
                        <tr>
...
<tr>
  ...
  <tr>
	...
	function cetak12() {
		...
		window.open('<?php echo site_url(); ?>/sspd_situ/cetak?sspd='+gabung, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	tabbar.setTabActive("a1");
		...
		/* function deleteData(){
			...
			if(confrm) {
				...
				statusLoading();
				dhtmlxAjax.post(base_url+'index.php/sspd_situ/delete', poStr, respeDel);
			}
		} */
		...
		var jumlahsitu = gridData.getRowsNum();
		 //alert(jumlahsitu);
		 var rekening;
		 var jumlah = 0;
		 var jumlah2 = 0;
		 
		for(var i=1;i <= jumlahsitu;i++){
		     rekening = gridData.cells(i,0).getValue();
			 //alert(rekening);
			 if(rekening=='4.1.1.04.01'){
				 var pokok = gridData.cells(i,8).getValue();
				 jumlah = parseInt(pokok) + jumlah;
				 }else if(rekening=='4.1.4.07.04'){
				 var denda = gridData.cells(i,8).getValue();
				 jumlah2 = denda + jumlah2;
			 }
		...
		tgl_masa2 = s5[2]+'/'+s5[1]+'/'+s5[0];	
            ...
            dhtmlxAjax.post(base_url+"index.php/sspd_situ/simpan", encodeURI(poststr), outputResponse);
	...
	function smsgateway(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sspd_situ/smsgateway', postStr, function(loader) {
			result = loader.xmlDoc.responseText;
			//alert(result);
			statusEnding();
		});
    ...
    var dpps = "";
	function initDP() {
		dpps = new dataProcessor(base_url+"index.php/sspd_situ/item_sspd");
		//dpps = new dataProcessor(base_url+"index.php/sspd_situ/item_nota");
		//dpps = new dataProcessor(base_url+"index.php/sspd_situ/item_skpd");
	...
	var dpps1 = "";
	function sendNota() {
		dpps1 = new dataProcessor(base_url+"index.php/sspd_situ/item_nota");
	...
	var dpps2 = "";
	function sendSkpd() {
		dpps2 = new dataProcessor(base_url+"index.php/sspd_situ/item_skpd");
...
//  statusEnding();
	...
	function getDataItem(){
		...
		gridData.clearAll();
		gridData.loadXML(base_url+"index.php/sspd_situ/load_rekening/"+gabung,function() {
			//loadtotal();
			statusEnding();
		});
	}
...
//		alert(jenis+"|"+keyword+"|"+parameter);
		gridWP.clearAll();	
		gridWP.loadXML(base_url+"index.php/sspd_situ/cariData/"+dasar+"/"+parameter+"/"+keyword,function(id) {  								
		statusEnding();
...
//		+kata_kunci+"/"+document.frmSrc4.parameter.value
	...
	function loadData(){
	gridDatas.clearAll();	
	gridDatas.loadXML(base_url+"index.php/sspd_situ/load_data",function() { 
           // statusEnding(); 
        });
        ...
        gridDatas.clearAll();	
	gridDatas.loadXML(base_url+"index.php/sspd_situ/load_data/"+parameter+"/"+kunci,function() { });
    }
	...
	function openChildsspd(){
		...
		gridData.clearAll();
		gridData.loadXML(base_url+"index.php/sspd_situ/load_childsspd/"+data);
	}
