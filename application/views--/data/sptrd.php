...
<h2 style="margin:30px 5px 25px 30px;">SPTRD - Surat Pemberitahuan Terhutang Retribusi Daerah</h2>
...
<div id="a_tabbar" style="width:1200px; height:730px; margin-left:30px;"></div>
  ...
  <br />
			<form name="frmData" id="frmData" method="post">
                ...
                <tr valign="top">
					<td width="25%" style="padding-left:15px;">No SPTRD</td>
                ...
                <tr>
                    ...
                    <td width="50" ><select name="lokasi" id="lokasi" onchange="getPasar();">
                         <option value=""></option>
                        <?php
                ...
                <tr>
					...
					<td><select name="txt_keperluan" id="txt_keperluan" onchange="getLokasi();" >
                         <option value=""></option>
                        <?php
                ...
                <tr>
                    ...
                    <select name="op" id="op" onchange="getGangguan();">
                         <option value=""></option>
                        <?php
...
//                                    if(isset($reklame)) { $opt="selected"; } else { $opt=""; }
                            ...
                            ?>
                    </select>&nbsp;&nbsp;Luas Ruang Tempat Usaha&nbsp;&nbsp;<input type="text" id="qty" name="qty" size="10" /> m<sup>2</sup>&nbsp;
                    <input type="button" value="hitung" onclick="perhitungan_retri()" name="hitung_ret" id="hitung_ret" disabled="disabled" style="padding-left:10px; padding-right:10px" />
                    &nbsp;Tarif&nbsp;&nbsp;<input type="text" id="harga" name="harga" disabled="disabled" size="18" />
                ...
                <tr>
                    ...
                    <!--<tr>
                        ...
                        <td colspan="4"><input type="hidden" size="5" name="sum" id="sum" /><input type="text" name="jumlah" id="jumlah" style="text-align:right" disabled/></td>
                    </tr>-->
                    <tr>
                    ...
                    <tr>
                  	...
                  	<tr>
						<td colspan="4" style="padding-left:15px; background-color:#FFCC99;"><input type="button" value="Baru" onclick="addNew()" name="btnaddnew" id="btnaddnew" style="width:90px;"/>&nbsp;&nbsp;
                        <input type="button" value="Simpan" onclick="simpanData()" name="btnsimpan" id="btnsimpan" style="width:90px;" disabled/>&nbsp;&nbsp;
                        <input type="button" value="Edit" onclick="editData()" name="btnedit" id="btnedit" style="width:90px;" disabled/>&nbsp;&nbsp;<input type="button" value="Hapus" onclick="deleteData()" name="btndel" style="width:90px;" id="btndel" disabled/>&nbsp;&nbsp;<input type="button" value="Cetak" onclick="cetak()" style="width:90px;" name="btncetak" id="btncetak" disabled/>&nbsp;&nbsp;</td>
					</tr>
...
var kelebihan_tarif = "";
	...
	function add(){
        ...
        var izin = document.frmData.lokasi.value;
        ...
        gride1.addRow(id,[row1,row2,row3,row4,row5,row6], posisi);
		gride1.showRow(id);
		var stage = 2;
		loadtotal(stage, id);
	...
	function loadtotal(stage, id) {
		//document.frmData.jumlah.value = document.getElementById('tmpHar').innerHTML;
		document.frmData.tarif.value = document.getElementById('tmpHar').innerHTML;
		document.frmData.setoran.value = document.getElementById('tmpHar').innerHTML;
	}
    ...
    function addRowItem(arrIndx) {       
		...
		//alert(arrs);
		var sptrd = document.frmData.sptpd.value; 
		var row1= arrs[0];
		var row2= arrs[1];
        var row3= arrs[2];
        var row4= arrs[3];
	...
	gride1.setHeader("Kode,Nama Rekening,Jenis Usaha,Luas tempat usaha (m2),Total tarif,stprd");
	gride1.setInitWidths("100,170,580,100,120,100");
	gride1.setColAlign("left,left,left,right,right,left");
	gride1.setColTypes("ro,ro,ro,ron,ron,ron");
	...
	gride1.attachFooter("Jumlah,#cspan,#cspan,<div style=text-align:right id=tmpVol>{#stat_total}</div>,<div style=text-align:right id=tmpHar>{#stat_total}</div>,<div style=text-align:right id=tmpDp>{#stat_total}</div>");
	...
	var dpps = "";
	function rundpPO(){
		dpps = new dataProcessor("<?php echo site_url(); ?>/sptrd/detail");
	...
	function lihat3() {
		...
		gridGAL.clearAll();
		gridGAL.loadXML("<?php echo site_url(); ?>/sptrd/load_perusahaan/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	grid.setHeader("id, no_sptrd, npwpd, nama_perusahaan, alamat, cara_hitung, tgl_diterima, indeks_lokasi, omset, petugas_input, masa_pajak1, masa_pajak2, Jumlah bayar,tarif,lokasi_pasar,indeks_gangguan,volume",null,["text-align:center","text-align:center","text-align:center","text-align:center"]);
	...
	grid.setInitWidths("50,150,150,170,100,100,100,100,100,100,100,100,100,100,100,100,100");
	grid.setColAlign("center,left,left,left,left,left,left,left,left,right,left,right,right,left,right,right,right");
	grid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ron,ro,ro,ron,ron,ron,ron,ron");
	grid.setColSorting("str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str");
	...
	grid.loadXML("<?php echo site_url(); ?>/sptrd/data_sptrd");
	grid.init();
	...
	function selectedOpenData(id) {
		...
		//return;
		//statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptrd/get_data_by_id', postStr, responePOST3);
		var sptrd = grid.cells(id,1).getValue();
        ...
        document.frmData.harga.value = format_number(grid.cells(id,12).getValue());
        ...
        document.frmData.op.value = grid.cells(id,15).getValue();
        document.frmData.qty.value = grid.cells(id,16).getValue();
        
		//alert(sptpd);
		gride1.clearAll();
		gride1.loadXML("<?php echo site_url(); ?>/sptrd/d_data?sptrd="+sptrd);
		//loadtotal();
        ...
        document.frmData.hitung_ret.disabled=true;
		tabbar.setTabActive("a1");
	}
	...
	function addNew() {
		gride1.clearAll();
        //document.frmData.skpd_p.disabled=false;
...
//		}
	...
	function cek(){
		...
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptrd/ricek', postCek, resPOST);
	}
	...
	function simpan2(){	
        ...
        "&qty=" + document.frmData.qty.value +
        "&gangguan=" + document.frmData.op.value +
		...
		+ "&usaha=" + document.frmData.lokasi.value
		...
		+ "&npwpd=" + document.frmData.txt_npwpd.value
		+ "&lokasi=" + document.frmData.txt_keperluan.value
	//	+ "&jenis_galian=" + document.frmData.txt_jenis_galian.value
        ...
        //var thn1 = document.getElementById("txt_thn_masa_pajak").value
        //var thn2 = document.frmData.txt_thn_masa_pajak.value
        //alert(thn1 + ',' + thn2);
		//alert('2...');
		...
		//return;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/sptrd/simpan_sptrd', postStr, responePOST2);
		  alert("Berhasil Simpan SPTRD");
		disabledData1();
	...
	function refreshData() {
		grid.clearAll();
		grid.loadXML(base_url+'index.php/sptrd/data_sptrd');
	}
		...
		if (name!=null){
			if(name=='passwordnyadpka'){
				...
				if(confrm) {
					...
					dhtmlxAjax.post(base_url+'index.php/sptrd/delete_sptrd', postStr, responePOST);
				}
    ...
    function perhitungan_retri(){                
            if(document.frmData.qty.value!=""){
			poststr = 'nilai_ret=' + document.frmData.qty.value;			           
			dhtmlxAjax.post(base_url+"/index.php/sptrd/getHitungan_ret", encodeURI(poststr), outputPerhitungan_ret);
			}		                                                   
    }
    
    function outputPerhitungan_ret(loader) {        
            result = loader.xmlDoc.responseText;
            
            if(result) {                                 
                //document.frmData.harga.value = result;
                indeks_tarif = result;
                var nilai =  document.frmData.qty.value;
                
                if (nilai > 100){
                    //alert('lebih dari 100');
                    kelebihan_nilai = nilai - 100;                                                                                    
                }else{
                    //alert('kurang dari 100');
                    kelebihan_nilai = 0;                    
                }
                
                if (kelebihan_nilai != 0) {
                    kelebihan_tarif = 1750;
                    var hasil = kelebihan_nilai * kelebihan_tarif;
                    //alert(hasil);
                }else{                    
                    kelebihan_tarif = 0;
                    var hasil = kelebihan_nilai * kelebihan_tarif
                ...
                var grand = parseFloat(indeks_tarif) + parseFloat(hasil);
                var grand2= grand*indeks_gangguan;
                var grand3= grand2*indeks_lokasi;
                //alert(indeks_lokasi + ' :lokasi , gangguan: ' + indeks_gangguan + ' , Hasil: '+indeks_tarif);                
                document.frmData.harga.value = grand3;
	...
	function getLastDate(iThn, iBln1, iBln2, iTgl1, iTgl2){				  
        ...
        var iThnVal2    = parseFloat(iThnVal) + 3;
		...
		document.getElementById(iTgl2).value = lastDateDay + '/' + iBln2Val + '/' + iThnVal2;	
	}
