...
<h2 style="margin:30px 5px 25px 30px;">Surat Tanda Bukti Pembayaran (STBP)<br/>Wajib Pajak Retribusi</h2>
    ...
    <form name="frmBayar" id="frmBayar">
    	<table border="0" id="tab_pengukuhan">
    		<tr>
                ...
                <td width="400"><input type="text" name="bayar" id="bayar" size="46px" style="text-transform:uppercase; background-color: #FFFFCC;" disabled="disabled"></td>
            </tr>
            <tr>
                <td width="174" style="padding-left:15px;">No. SSRD</td>
                <td width="400"><input type="text" name="ssrd" id="ssrd" size="34px" style="text-transform:uppercase; background-color: #FFCC99;" disabled="disabled"><input type="button" value="Cari" onclick="openData()" name="buka" style="padding-left:20px; padding-right:20px" disabled="disabled"/></td>
            </tr>
            <tr>
    			<td style="padding-left:15px;">NPWPD Perusahaan</td>
				<td><input type="text" name="npwpd" id="npwpd" disabled="disabled" size="46px" style="background-color:#FFFFCC;"/><input type="hidden" name="skrd" id="skrd" disabled="disabled" size="46px" style="background-color:#FFFFCC;"/></td>
         	</tr>
            <tr>
            ...
            <tr>
                ...
                <input type="hidden" name="no_sptrd" id="no_sptrd" /></td>
    		</tr>
            <tr>
                <td style="padding-left:15px;">Kurang</td>
				<td><input type="text" name="kurang_wp" id="kurang_wp" disabled="disabled" size="35px" /></td>
    		</tr>
            <tr>
                <td style="padding-left:15px;">lebih</td>
				<td><input type="text" name="lebih_wp" id="lebih_wp" disabled="disabled" size="35px" /></td>
    		</tr>
            <tr>
...
<script type="text/javascript">
	function cetakData() {
		window.open('<?php echo site_url(); ?>/buku2/cetak_bayar?bayar='+document.frmBayar.bayar.value, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	function deleteData(){
		...
		if(confrm) {
			...
			statusLoading();
			dhtmlxAjax.post(base_url+'index.php/buku2/deleteBayar', postStr, responeDel);	
		}
	}
	...
	grid.setHeader("id, no_tbp, no_ssrd, npwpd, skrd, nama_perusahaan, alamat_perusahaan, jumlah, setoran, kurang, lebih");
	...
	grid.loadXML("<?php echo site_url(); ?>/buku2/data_pembayaran");
	grid.init();
	...
	function selectedOpenData(id){
		...
		document.frmBayar.ssrd.value = grid.cells(id,2).getValue();
		document.frmBayar.npwpd.value = grid.cells(id,3).getValue();
		document.frmBayar.skrd.value = grid.cells(id,4).getValue();
	...
	function simpan() {
		...
		if(document.frmBayar.ssrd.value=="") {
			alert("No.SSRD Tidak Boleh Kosong");
			document.frmBayar.ssrd.focus();
			return;
		}
		...
		var postStr =
			...
			"&ssrd=" + document.frmBayar.ssrd.value +
			"&skrd=" + document.frmBayar.skrd.value +
            ...
            "&sptrd=" + document.frmBayar.no_sptrd.value;
            ;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/buku2/simpan_bayar', postStr, responePOST);
	}
	...
	function lihat21() {
		...
		gridData.clearAll();
		gridData.loadXML("<?php echo site_url(); ?>/buku2/load_ssrd/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	gridData.setHeader("id,no_ssrd,tanggal,no_skrd,npwpd,nama_perusahaan,alamat_perusahaan,ketetapan,setoran,sptrd");
	...
	function selectedOpenData21(id){
		document.frmBayar.ssrd.value = gridData.cells(id,1).getValue();
		document.frmBayar.npwpd.value = gridData.cells(id,4).getValue();
		document.frmBayar.skrd.value = gridData.cells(id,3).getValue();
        ...
        document.frmBayar.no_sptrd.value = gridData.cells(id,9).getValue();
        alert(gridData.cells(id,3).getValue());
	...
	function barus(){
		...
		document.frmBayar.ssrd.disabled = false;
		document.frmBayar.ssrd.focus();
		document.frmBayar.tambah.disabled = false;              
	}
	...
	function kosongfrmData() {
		...
		document.frmBayar.skrd.value = "";
		document.frmBayar.ssrd.value = "";
