...
<script language="javascript">
	...
	function simpan() {
		...
		var postStr =
			...
			//"&pajak=" + pajak;
		statusLoading();
		dhtmlxAjax.post(base_url+'index.php/surat_perpanjangan/simpan', postStr, responePOST);
	}
...
<h2 style="margin:30px 5px 25px 30px;">Surat Perpanjangan Pajak Reklame</h2>
  ...
  <div id="C1" style="background-color: #B3D9F0;"><br />
    <form name="frmPengukuhan" id="frmPengukuhan">
    	<table border="0" id="tab_pengukuhan">
    		<tr>
                <td width="174" style="padding-left:15px;">No.</td>
...
<script language="javascript">
	function cetakData(){
		window.open('<?php echo site_url(); ?>/surat_perpanjangan/cetak?npwpd='+document.frmPengukuhan.npwpd.value, '', 'height=700,width=1000,scrollbars=yes');
	}
	...
	function lihat21() {
		...
		gridData.clearAll();
		gridData.loadXML("<?php echo site_url(); ?>/surat_perpanjangan/load_perusahaan/"+op+"/"+nilai,function() {
			statusEnding();
		});
	}
	...
	grid.setHeader("ID, No. Teguran, NPWPD Perusahaan, Tanggal Surat, Nama Perusahaan, Alamat Perusahaan, Pemilik/Pimpinan"); //7
	grid.setInitWidths("50,110,120,120,150,300,100");
	grid.attachHeader("#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter");
	grid.setColAlign("center,center,center,center,left,left,left");
	...
	grid.loadXML("<?php echo site_url(); ?>/surat_perpanjangan/data");
	grid.init();
	...
	function selectedOpenData(id) {
		...
		document.frmPengukuhan.nama.value = grid.cells(id,4).getValue();
		document.frmPengukuhan.alamat.value = grid.cells(id,5).getValue();
		document.frmPengukuhan.pemilik.value = grid.cells(id,6).getValue();
		//menu = grid.cells(id,6).getValue();
        //document.frmPengukuhan.usaha.value = grid.cells(id,7).getValue();
		
		// document.frmPengukuhan.tunggakan.value = grid.cells(id,8).getValue();
		/* arr_menu = menu.split('|');
		jml = <?php echo $jmlOL; ?>;
		for(p=0;p<jml;p++) {
			<?php for($i=1;$i<=$jmlOL;$i++) { ?>
				...
				if(confrm) {					
						var postStr =
							...
							"&npwpd=" + document.frmPengukuhan.npwpd.value;		
								//alert(postStr);
							statusLoading();
							dhtmlxAjax.post(base_url+'index.php/surat_perpanjangan/delete', postStr, responeDel);	
				}
			} else {
