...
where a.no_skpd='".$_GET['skpd']."'")->row();
		...
		$report .=
			...
			foreach($child->result() as $ch) {
										...
										if($ch->tarif == 45000){
											$cetakBulan = "";
											$durasiBulan = "x ".$selisihBulan."Bln";
										}else{
											$durasiBulan = "";
										...
										if(strpos(strtoupper($data->ket_skpd),'ANGSURAN')>0){
											$angsuran = "";
										}else{
											if($ch->nsl==1) $enesel = "";
											else $enesel = $ch->nsl.' x ';
										...
										$angsuran= $ch->panjang.' x '.$ch->lebar.'M x '.$ch->sisi.'SISI x '.$ch->unit.'BH '.$cetakHari.' x Rp '.number_format($ch->tarif,0,",",".").' x '.$ch->nsl.' x '.$ch->trf.'% '.$cetakBulan.' <br/>' ;
					...
					$report .= '<tr id='.$i.'>
					  ...
					  <td width="302" height="25" align="left" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">'.$ch->tema.'<br/>'.$angsuran.' Lokasi: '.$ch->lokasi1.'<br/> TMT : '.$masaPajak.'</td>
			...
			$report .=
				'<tr style="font-family:Times new roman; font-size:12px; font-weight:bold;">
					  <td colspan="3" height="25" width="472" align="right">'.$keteranganKekurangan.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
