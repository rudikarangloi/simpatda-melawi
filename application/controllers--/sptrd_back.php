...
class sptrd extends MY_Controller {
	...
	function d_data(){
		$grid = new GridConnector($this->db->conn_id);
		if(isset($_GET['sptrd'])) {
			$grid->render_sql("select kd_rek, nm_rek, volume, harga_pasar, dp, tarif, jumlah, sptrd from sptrd_d1 where sptrd = '".$_GET['sptrd']."'","id","kd_rek, nm_rek, volume, harga_pasar, dp, tarif, jumlah, sptrd");
		} else {
	...
	public function data_sptrd() {
		...
		$grid->render_sql("select a.id, a.no_sptrd, a.npwpd, b.nama_perusahaan, b.alamat_perusahaan,
         a.cara_hitung, a.tgl_diterima, a.keperluan, a.omset, a.petugas_input, a.masa_pajak1,
          a.masa_pajak2, a.jml_bayar, a.tarif,a.lokasi_pasar from sptrd_pba a left join identitas_perusahaan b 
          on a.npwpd=b.npwpd_perusahaan left join sptrd c on c.no_sptrd = a.no_sptrd ","id","id, no_sptrd,
           npwpd, nama_perusahaan, alamat_perusahaan, cara_hitung, tgl_diterima, keperluan, omset,
            petugas_input, masa_pajak1, masa_pajak2, jml_bayar,tarif,lokasi_pasar");	
	}
...
where a.author = '".$this->session->userdata('username')."'","id","id, no_sptpd, npwpd, nama_perusahaan, alamat_perusahaan, cara_hitung, tgl_diterima, keperluan, jenis_galian, omset, petugas_input, masa_pajak1, masa_pajak2, jml_bayar,tarif");	
	...
	public function get_data_by_id(){
		$query = $this->db->query("select a.id, a.no_sptrd, a.npwpd, b.nama_perusahaan, b.alamat_perusahaan,
         a.cara_hitung, a.tgl_diterima, a.keperluan, a.masa_pajak1, a.masa_pajak2, a.author, a.created,
          a.modified, a.petugas_input from sptrd_pba a left join identitas_perusahaan b
	...
	function cetak(){
		$sql = $this->db->query("select sptrd_pba.id, sptrd_pba.npwpd, sptrd_pba.no_sptrd,
         ...
         sptrd_pba.keperluan,DATE_FORMAT(sptrd_pba.tgl_diterima,'%d/%m/%Y') as tgl_diterima, sptrd_pba.petugas_input,
