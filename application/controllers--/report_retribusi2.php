...
class report_retribusi2 extends MY_Controller {
...
$report .=
	...
	function rekon_bank(){
		...
		$this->load->view('report_retribusi/rekon_bank', $data);
	}
	
	function data_bank($tgl=""){
...
//			$qr="";
//		} else {
//			$qr="and ssrd.kode_pajak='".$pajak."'";
//		}
		
		$query = $this->db->query("SELECT
ssrd.nomor,
ssrd.no_ssrd,
DATE_FORMAT(ssrd.tanggal,'%d/%m/%Y') AS tgl2,
...
FROM
ssrd
INNER JOIN view_perusahaan ON ssrd.npwpd = view_perusahaan.npwpd_perusahaan
INNER JOIN pembayaran_ret ON pembayaran_ret.no_ssrd=ssrd.no_ssrd where pembayaran_ret.tgl_bayar = '".$tgl."'");
		...
		foreach($query->result() as $rs){
			$no = $rs->nomor;
				...
				$pajak = 'RETRIBUSI';
                $spt = $this->db->query("SELECT a.no_sptrd, DATE_FORMAT(a.tgl_diterima,'%d/%m/%Y') AS tgl, b.kd_rek, b.nm_rek,
                 DATE_FORMAT(a.masa_pajak1,'%d/%m/%Y') AS masa1, DATE_FORMAT(a.masa_pajak2,'%d/%m/%Y') AS masa2 FROM sptrd_pba a
                  LEFT JOIN sptrd_d1 b ON a.no_sptrd=b.sptrd LEFT JOIN skrd c ON a.no_sptrd=c.no_sptrd WHERE  c.no_skrd = '".$no."'")->row();
			...
			$piutang = $this->db->query("SELECT SUM(ssrd.ketetapan) AS tetap, SUM(ssrd.denda) AS denda, SUM(pembayaran_ret.setoran_wp) AS jumlah FROM ssrd
LEFT JOIN pembayaran_ret ON pembayaran_ret.no_ssrd=ssrd.no_ssrd where ssrd.nomor = '".$no."' and ssrd.tahun_pajak != '".$thn."'")->row();
			$j = $piutang->jumlah;
			$t = $piutang->tetap;
			$d = $piutang->denda;
			$piu = $j-($t+$d);
			...
			echo ("<row id='".$i."'>");
				...
				echo("<cell><![CDATA[".$spt->no_sptrd."]]></cell>");
...
ssrd
INNER JOIN view_perusahaan ON ssrd.npwpd = view_perusahaan.npwpd_perusahaan
INNER JOIN pembayaran_ret ON pembayaran_ret.no_ssrd=ssrd.no_ssrd where sspd.pembayaran = '".$bank."' and sspd.tanggal = '".$tgl."' $qr");
		...
		foreach($query->result() as $rs){
			...
			$piutang = $this->db->query("select sum(ketetapan) as tetap, sum(denda) as denda, sum(setoran) as jumlah from sspd where nomor = '".$no."' and tahun_pajak != '".$thn."'")->row();
			$j = $piutang->jumlah;
			$t = $piutang->tetap;
			$d = $piutang->denda;
			$piu = $j-($t+$d);
...
INNER JOIN target_pajak ON master_sptpd.kode_sptpd = target_pajak.jenis_pajak");
		...
		foreach($sql->result() as $rs) {
			...
			$realisasi = $this->db->query("select sum(setoran) as jumlah from sspd where kode_pajak = '".$kode."' and tahun_pajak ='".date('Y')."' and tanggal <= '".date('Y-m-d')."'")->row();
...
$terima2 = $query2->jl;
	...
	function data_setor($tgl="",$pajak=""){
		...
		$query = $this->db->query("select a.no_sspd, a.npwpd, b.nama_perusahaan, b.alamat_perusahaan, a.masa_pajak1, a.masa_pajak2, a.tahun_pajak, a.ketetapan, a.denda, a.setoran from sspd a left join view_perusahaan b on a.npwpd=b.npwpd_perusahaan where a.tanggal = '".$tgl."' $qr");
	...
	function cetak_setor(){
		...
		$query = $this->db->query("select a.no_sspd, a.npwpd, b.nama_perusahaan, b.alamat_perusahaan, a.masa_pajak1, a.masa_pajak2, a.tahun_pajak, a.ketetapan, a.denda, a.setoran from sspd a left join view_perusahaan b on a.npwpd=b.npwpd_perusahaan where a.tanggal = '".$gl."' $qr");
		...
		$report .=
			'<table border="1">
				<tr>
					...
					<td width="70" height="12" align="center"><strong>NPWPD</strong></td>
					<td width="150" height="12" align="center"><strong>NAMA WAJIB PAJAK</strong></td>
					<td width="150" height="12" align="center"><strong>ALAMAT</strong></td>
					<td width="120" height="12" align="center"><strong>KECAMATAN</strong></td>
		...
		foreach($query->result() as $rs) {
			...
			$report .=
				'<tr>
					...
					<td width="70" height="12" align="center">'.$rs->npwpd_perusahaan.'</td>
					<td width="150" height="12" align="left">&nbsp;&nbsp;'.$rs->nama_perusahaan.'&nbsp;</td>
					<td width="150" height="12" align="left">&nbsp;&nbsp;'.$rs->alamat_perusahaan.'&nbsp;</td>
					<td width="120" height="12" align="left">&nbsp;&nbsp;'.$rs->nama_kecamatan.'&nbsp;</td>
					<td width="120" height="12" align="left">&nbsp;&nbsp;'.$rs->nama_kelurahan.'&nbsp;</td>
					<td width="120" height="12" align="left">&nbsp;&nbsp;'.$rs->nama_sptpd.'&nbsp;</td>
				</tr>';
			$i++;
		}
