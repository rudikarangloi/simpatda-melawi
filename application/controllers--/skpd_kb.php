...
class skpd_kb extends MY_Controller {
	...
	public function index() {
		...
		$this->load->view('penetapan/skpd_kb',$data);
	}
	
	public function hitung($idtype="") {
   	...
   	public function hitung_reklame($idtype="") {
		$s = $this->db->query("select * from menu_modul where id_modul='".$this->session->userdata('id_modul')."'")->row();
		$data['nm_modul'] = $s->nm_modul;
		$p = $this->db->query("select nama_sptpd from master_sptpd where kode_sptpd = '".$idtype."'")->row();
		$data['nama_sptpd'] = $p->nama_sptpd;
		...
		$data['username'] = $username = $this->session->userdata('username');
		$data['tahun'] = $this->msistem->tahun();
		$this->load->view('penetapan/skpd_kb_rek',$data);
	}
...
where nota_hitung.nama_sptpd = '".$kode_sptpd."' and nota_hitung.status ='0' $qr","id","no_nota, sptpd, npwpd, nama_perusahaan ,alamat_perusahaan, nama_pemilik, alamat_pemilik, masa_pajak1, masa_pajak2, tahun, jumlah, masa_pajak_1, masa_pajak_2, tgl_tempo, tgl, ket_nhp, kep_proyek");
	...
	function mainData() {
		...
		$grid->render_sql("SELECT skpd.no_skpd, DATE_FORMAT(skpd.tgl,'%d/%m/%Y') AS tgl,
         DATE_FORMAT(skpd.tgl_jth_tempo,'%d/%m/%Y') AS tgl_jth_tempo, identitas_perusahaan.nama_perusahaan,
          identitas_perusahaan.alamat_perusahaan , identitas_pemilik.nama_pemilik, identitas_pemilik.alamat_pemilik, 
          ...
          FROM skpd LEFT JOIN identitas_perusahaan ON skpd.npwpd=identitas_perusahaan.npwpd_perusahaan LEFT JOIN
		  identitas_pemilik ON identitas_perusahaan.npwpd_pemilik=identitas_pemilik.npwpd_pemilik
          where skpd.kode_sptpd = '".$_GET['kode']."' ORDER BY skpd.no_skpd","id","no_skpd, tgl, nama_perusahaan, alamat_perusahaan,
	...
	function dataItemPajak1($s=""){
		...
		if($so[1]=="REK"){
			$wo = $this->db->query("SELECT  a.kd_rek,a.nm_rek,a.dp,b.trf AS tarif,a.kenaikan,a.denda,a.bunga,a.kompensasi,b.ketetapan AS jumlah FROM nhp_child a
	...
	public function cetak() {		
			...
			a.nota_hitung, a.no_kohir, a.id_ttd, a.masa_pajak_1,a.masa_pajak_2,a.ket_skpd, c.nama_kelurahan FROM skpd a 
		...
		$report .=
			'<table border=1>
				<tr>
					<td width="50" height="54" align="center" valign="middle"><img src="./images/Logo Daerah Jambi2.png" width="50" height="59" /></td>
					...
					<td width="210" align="center" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; ">
						<b>SURAT KETETAPAN PAJAK DAERAH KURANG BAYAR<br />
						PAJAK '.$nama_sptpd.'<br/>
		...
		$report .=
			'<table border="1">
				...
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">
					...
					<td width="10" align="center">:</td>
					<td width="290">&nbsp;&nbsp;'.$alamat.$kelurahan.'</td>
				</tr>
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">
		...
		$report .=
			...
			'<table border="1">
				...
				<td width="272">									
					...
					<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">	
						<td width="272" align="center">a.n Kepala Dinas Pendapatan Kota Jambi <br/>'.$jab1.'</td>
					</tr>					
					<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">
...
where a.no_skpd='".$_GET['skpd']."'")->row();
			...
			'<table border=1>
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">
					...
					<td width="210" align="center">
						SURAT KETETAPAN PAJAK DAERAH KURANG BAYAR<br />
						PAJAK '.$nama_sptpd.'<br />
						Masa Pajak&nbsp;:<br/>'.$tms_pajak1_thn.' - '.$tms_pajak2_thn.'						
					</td>
		...
		$report .=
			'<table border="1">
				...
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">
					...
					<td width="10" align="center">:</td>
					<td width="300">&nbsp;&nbsp;'.$alamat.$kelurahan.'</td>
				</tr >
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">
					<td width="160">&nbsp;&nbsp;NAMA PEMILIK</td>
					<td width="10" align="center">:</td>
					<td width="190">&nbsp;&nbsp;'.$namper.'</td>
				</tr>
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">
		...
		$report .=
			'<table border="1">
				<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">
					...
					<td width="100" height="20" align="center">Jumlah</td>
				</tr>';
			...
			foreach($child->result() as $ch) {
										...
										if(strpos(strtoupper($data->ket_skpd),'SKP-KB')==0){
											$SKPDKB = "";
										}else{
										$SKPDKB= $ch->panjang.' x '.$ch->lebar.'M x '.$ch->sisi.'SISI x '.$ch->unit.'BH '.$cetakHari.' x Rp '.number_format($ch->tarif,0,",",".").' x '.$ch->nsl.' x '.$ch->trf.'% '.$cetakBulan.' <br/>' ;
										}
					...
					$report .= '<tr id='.$i.'>
					  ...
					  <td width="302" height="25" align="left" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">'.$ch->tema.'<br/>'.$SKPDKB.' Lokasi: '.$ch->lokasi1.'<br/> TMT : '.$masaPajak.'</td>
					  <td width="100" height="25" align="right" style="font-family: Times new roman; font-size:12px; font-weight:bold;">Rp. '.number_format($ch->jumlahh,2,',','.').'&nbsp;&nbsp;</td>
					</tr>';
					$i++;
			...
			$report .=
				'<tr style="font-family: Times new roman; font-size:12px; font-weight:bold;">
					  ...
					  <td width="100" height="25" align="right">Rp. '.number_format($JumlahKetetapan,2,',','.').'&nbsp;&nbsp;</td>
				</tr>
				
				<tr style="font-family:Times new roman; font-size:12px; font-weight:bold;">
					  <td colspan="3" height="25" width="472" align="right">Jumlah Sanksi/Denda&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
					  <td width="100" height="25" align="right">Rp. '.number_format($ch->denda,2,',','.').'&nbsp;&nbsp;</td>
				</tr>
				
				<tr style="font-family: Times new roman; font-size:12px; font-weight:bold;">
					  <td colspan="3" height="25" width="472" align="right">Jumlah Keseluruhan&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
					  <td width="100" height="25" align="right">Rp. '.$total.'&nbsp;&nbsp;</td>
				</tr>
				</table>';
			...
			$report .=
					...
					<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">
						...
						<table border="0">
							<tr>
								...
								<td width="420" height="15"><i>'.$terbilang.'RUPIAH</i></td>
							</tr>							
						</table>
						</td>
					</tr>
				...
				$kenaikan= $ch->denda / $ch->jumlahh * 100 ;
				//keterangan
				if($ch->denda == 0 ){
					$ket_denda = $data->ket_skpd ;
				}else{
					$ket_denda = 'Denda : Rp. '.number_format($ch->jumlahh,2,",",".").' x '.$kenaikan.'% = '.number_format($ch->denda,2,",",".");
					}
			$report .=			
			'			
			<table border="1">
			...
			'<table border="1">
				...
				<td width="300">									
					...
					<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">						
						<td width="10">&nbsp;</td>						
						<td width="20">1.</td>						
						<td width="230" align="justify">Harap penyetoran dilakukan pada Bank / Bendahara Penerimaan Dinas Pendapatan Kota Jambi.</td>						
					</tr>
					<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">						
						<td width="10">&nbsp;</td>						
						<td width="20">2.</td>						
				...
				<td width="272">									
					...
					<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">	
						<td width="272" align="center">an. Kepala Dinas Pendapatan Kota Jambi <br/>'.$jab1.'</td>
					</tr>					
					<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">
						<td width="272" height="40">&nbsp;</td>
					</tr>
	...
	public function cetak_skr() {		
		$data = $this->db->query("SELECT a.no_skpd, d.no_sptpd,DATE_FORMAT(a.tgl,'%d/%m/%Y') AS tgl, DATE_FORMAT(a.tgl_jth_tempo,'%d/%m/%Y') AS tgl_jth_tempo,
d.masa_pajak_1 AS msp1,d.masa_pajak_2 AS msp2,b.nama_perusahaan, 
b.alamat_perusahaan, b.nama_pemilik, b.alamat_pemilik, b.npwpd_perusahaan, b.npwpd_lama,c.denda,c.jumlah, a.jumlah AS total, a.masa_pajak1, 
a.masa_pajak2, a.tahun, a.no_sptpd, a.nota_hitung, 
a.no_kohir,a.id_ttd, e.nama_kelurahan FROM skpd a 
LEFT JOIN view_perusahaan b ON a.npwpd=b.npwpd_perusahaan
LEFT JOIN sptpd d ON d.no_sptpd = a.no_sptpd
LEFT JOIN skpd_child c ON c.skpd = a.no_skpd LEFT JOIN view_kelurahan e ON SUBSTR(b.npwpd_perusahaan,19,2)=e.kode_kelurahan AND SUBSTR(b.npwpd_perusahaan,16,2)=e.kode_kecamatan
		...
		$report .=
			...
			foreach($child->result() as $ch) {
                                        ...
                                        if($ch->kd_rek_sptpd=='4.1.1.04.02'){
                                            ...
                                            $pajak1 = date('d-m-Y', strtotime($ch->ms_pajak1));
                                            $pajak2 = date('d-m-Y', strtotime($ch->ms_pajak2));  
											$selisihBulan = round(((abs(strtotime ($pajak2) - strtotime ($pajak1)))/(60*60*24*30)));
											//jika selisih bulan habis dibagi duabelas (1 tahun) maka
											//selisih bulan dibagi 12 dapat berapa tahun
											//jika 1 tahun tidak ditulis
											// jika lebih 1 tahun tulis
											if($selisihBulan%12==0){
												$selisihTahun = $selisihBulan/12;
												if($selisihTahun>1){
													$cetakHari = "x ".$selisihTahun."Thn";
												}else{
													$cetakHari = "";
												}
											}else{
												$cetakHari = ": 12Bln x ".$selisihBulan."Bln";
											}
					...
					$report .= '<tr id='.$i.'>
					  <td width="25" height="25" align="center" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">'.$i.'</td>
					  <td width="145" height="25" align="center" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">4.1.2.02.01</td>
					  <td width="302" height="25" align="left" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">Pemakaian Kekayaan Milik Pemda<br/> - Sewa Panggung Bilboard</td>
					  <td width="100" height="25" align="right" style="font-family: Times new roman; font-size:12px; font-weight:bold;">Rp. '.number_format($ch->jumlahh,2,',','.').'&nbsp;&nbsp;</td>
					</tr>';
					$i++;
					$JumlahKetetapan+=$ch->jumlahh;
			...
			<table border="1">
				...
				<table border="0">
					<tr style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;">
						<td height="15" width="5">&nbsp;</td>
						<td height="15">&nbsp;K E T : '.$ket_denda.' </td>
					</tr>					
				</table>
				</td>
				</tr>
