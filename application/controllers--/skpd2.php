...
where nota_hitung.nama_sptpd = '".$kode_sptpd."' and nota_hitung.status ='0' $qr","id","no_nota, sptpd, npwpd, nama_perusahaan ,alamat_perusahaan, nama_pemilik, alamat_pemilik, masa_pajak1, masa_pajak2, tahun, jumlah, masa_pajak_1, masa_pajak_2, tgl_tempo, tgl, ket_nhp, kep_proyek");
	...
	public function cetak() {		
		...
		if($sptpd=='HTL'){
			...
			$ketTambahan = "DP = ".$volumeAir. " m<sup>3</sup> x ".$indeksAir." x Rp. ".$hargaDasarAir." = Rp. ".number_format($hargaDasarHasil,0,',','.');
			$ketNoUrut = "<tr style='font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-weight:bold;'>
		...
		$report .=
			...
			foreach($child->result() as $ch) {
					...
					$report .= '<tr id='.$i.'>
					  ...
					  <td width="302" height="25" align="left" style="font-family: Arial,Helvetica Neue,Helvetica,sans-serif; font-size:11px; font-weight:bold;">'.$ch->tema.'<br/>'.$ch->panjang.' x '.$ch->lebar.'M x '.$ch->sisi.'SISI x '.$ch->unit.'BH '.$cetakHari.' x Rp '.number_format($ch->tarif,0,",",".").' x '.$ch->nsl.' x '.$ch->trf.'% '.$cetakBulan.' <br/> Lokasi: '.$ch->lokasi1.'<br/> TMT : '.$masaPajak.'</td>
