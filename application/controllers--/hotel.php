...
class hotel extends MY_Controller {
	...
	public function index() {
		...
		$data['gresto'] = $this->db->query("select kd_rek,concat(kd_rek, ' | ',nm_rek) as nm_rek from master_rekening where kd_rek = '4.1.1.01.12' and status_aktif ='1'");
		...
		$pemilik = "";
		$perusahaan = "";
		$this->load->view('data/hotel',$data);
	}
...
LEFT JOIN master_rekening c ON c.pajak = LEFT(a.npwpd_perusahaan,1) AND c.pajak_det = SUBSTRING(a.npwpd_perusahaan,9,2) WHERE a.status_aktip='Y' AND LEFT(a.npwpd_perusahaan,2)='01' AND $cu");
	...
	public function data() {
		...
		$grid->render_sql("select sptpd_hotel.id, 
sptpd_hotel.npwpd,
...
sptpd_child.tarif,
sptpd_hotel.ket_pajak,
sptpd_hotel.ket_inap
FROM
identitas_perusahaan
INNER JOIN sptpd_hotel ON sptpd_hotel.npwpd = identitas_perusahaan.npwpd_perusahaan
INNER JOIN sptpd_child ON sptpd_hotel.no_sptpd = sptpd_child.no_sptpd order by no_sptpd DESC","id","id, npwpd, no_sptpd, nama_perusahaan, alamat_perusahaan, jns, ket_insidentil, cara_hitung, tgl_diterima, petugas, gol_hotel, masa_pajak1, masa_pajak2, omset, jml_bayar, tarif,ket_pajak,ket_inap");	
	}
...
INNER JOIN sptpd_hotel ON sptpd_hotel.npwpd = identitas_perusahaan.npwpd_perusahaan INNER JOIN sptpd_child ON sptpd_hotel.no_sptpd = sptpd_child.no_sptpd where sptpd_hotel.author = '".$this->session->userdata('username')."'","id","id, npwpd, no_sptpd, nama_perusahaan, alamat_perusahaan, jns, ket_insidentil, cara_hitung, tgl_diterima, petugas, gol_hotel, masa_pajak1, masa_pajak2, omset, jml_bayar,tarif");	
	...
	public function simpan() {
		...
		$data = array (
			...
			//'nama_perusahaan' 		=> strtoupper($this->input->post('nama')),
			...
			'gol_hotel' 	=> strtoupper($this->input->post('gol')),
		...
		if($this->input->post('id')==0){
			...
			$dataIns = array_merge($data,array('no_sptpd' => $sptpd,'created' => date('Y-m-d H:i:s')));
			$this->db->insert('sptpd_hotel',$dataIns);
			...
			//table sptpd child
			$dataIns3 = array_merge($data_child,array('no_sptpd' => $sptpd));
			$this->db->insert('sptpd_child',$dataIns3);
			
		} else {
			$sptpd = $this->input->post('sptpd');
			$dataUpd = array_merge($data,array('no_sptpd' => $sptpd,'modified' => date('Y-m-d H:i:s')));
			$this->db->update('sptpd_hotel', $dataUpd, array('id' => $this->input->post('id')));
			...
			//table sptpd child
			$dataUpd3 = array_merge($data_child,array('no_sptpd' => $sptpd));
	...
	public function delete() {
		...
		$sa = $r->jml;
		if($sa==0){
			$this->db->delete('sptpd_hotel',array('no_sptpd' => $this->input->post('sptpd')));
			$this->db->delete('sptpd',array('no_sptpd' => $this->input->post('sptpd')));
			$this->db->delete('sptpd_child',array('no_sptpd' => $this->input->post('sptpd')));
