...
dhtmlXGridObject.prototype.collectValues=function(column){
	var c={}; var f=[];
	this._build_m_order();		
	var col=this._srowsCol||this.rowsCol;
	if (col.length)
		var e=this.cells3(col[0],column);
	...
	var col=this._srowsBuf||this.rowsBuffer;
...
dhtmlXGridObject.prototype._in_header_stat_multi_total=function(t,i,c){
	...
	var calck=function(){
		var summ=0;
		for (var j=0; j<this.rowsCol.length; j++){
			var v=parseFloat(this.cells2(j,cols[0]).getValue())*parseFloat(this.cells2(j,cols[1]).getValue());
			summ+=isNaN(v)?0:v;
		}
		return this._maskArr[i]?this._aplNF(summ,i):(Math.round(summ*100)/100);
	}
