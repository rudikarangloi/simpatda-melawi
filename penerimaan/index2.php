...
$total_setoran = 0;
	  ...
	  while($rekening_detail = mysql_fetch_assoc($query_rekening_detail)){
		 ...
		 $sspd = mysql_fetch_assoc(mysql_query("SELECT SUM(denda) AS denda, SUM(ketetapan) AS ketetapan FROM sspd WHERE kode_bank = '2' AND tanggal = '$tanggal' AND SUBSTR(npwpd,1,1) = '".(int)$kode_jenis."' AND SUBSTR(npwpd,9,2) = '$kode_rekening_detail'"));
		 $denda += $sspd['denda'];
